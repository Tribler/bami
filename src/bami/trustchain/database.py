...
from bami.trustchain.blockcache import BlockCache
...
class TrustChainDB(Database):
    ...
    def get_number_of_known_blocks(self, public_key=None):
        ...
        if public_key:
            return list(
                self.execute(
                    u"SELECT COUNT(*) FROM blocks WHERE public_key = ?", (public_key,)
                )
            )[0][0]
    ...
    def remove_old_blocks(self, num_blocks_to_remove, my_pub_key):
        ...
        self.execute(
            ...
            (my_pub_key, my_pub_key, num_blocks_to_remove),
        )
    ...
    def get_blocks_with_type(self, block_type, public_key=None):
        ...
        if public_key:
            return self._getall(
                u"WHERE type = ? and public_key = ?", (block_type, public_key)
            )
    ...
    def get_lowest_range_unknown(self, public_key):
        ...
        db_result = list(
            self.execute(query, (public_key, lowest_unknown), fetch_all=True)
        )
        if db_result:
    ...
    def get_upgrade_script(self, current_version):
        ...
        if current_version <= 6:
            ...
            """ % self.get_sql_create_blocks_table(
                "blocks", "public_key, sequence_number, block_hash"
            )
