...
from bami.trustchain.block import (
    ANY_COUNTERPARTY_PK,
    EMPTY_PK,
    GENESIS_SEQ,
    TrustChainBlock,
...
from bami.trustchain.caches import (
    ChainCrawlCache,
    CrawlRequestCache,
    HalfBlockSignCache,
    IntroCrawlTimeout,
)
from bami.trustchain.database import TrustChainDB
from bami.trustchain.payload import *
from bami.trustchain.settings import TrustChainSettings
...
class TrustChainCommunity(Community):
    ...
    async def process_half_block(self, blk, peer):
        ...
        if (
            ...
            blocks = self.sign_block(
                peer, linked=blk, additional_info=self.get_counter_tx(blk)
            )
    ...
    def create_introduction_request(
        self, socket_address, extra_bytes=b"", new_style=False
    ):
