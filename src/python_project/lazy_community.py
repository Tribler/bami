...
def lazy_wrapper(*payloads):
    ...
    def decorator(func):
        ...
        def wrapper(self, source_address, data):
            # UNPACK
            auth, remainder = self.serializer.unpack_to_serializables(
                [BinMemberAuthenticationPayload,], data[23:]
            )
...
class EZPackOverlay(Overlay):
    ...
    def _ez_senda(self, address, *payloads, **kwargs):
        ...
        """
        self.endpoint.send(
            address, self.ezr_pack(payloads[-1].msg_id, *payloads, **kwargs)
        )
    ...
    def _verify_signature(self, auth, data):
        ...
        return (
            ec.is_valid_signature(public_key, data[:-signature_length], signature),
            remainder,
        )
    ...
    def _ez_unpack_auth(self, payload_class, data):
        ...
        if not signature_valid:
            raise PacketDecodingError(
                "Incoming packet %s has an invalid signature" % payload_class.__name__
            )
        # PRODUCE
    ...
    def _ez_unpack_noauth(self, payload_class, data, global_time=True):
        # UNPACK
        format = (
            [GlobalTimeDistributionPayload, payload_class]
            if global_time
            else [payload_class]
        )
