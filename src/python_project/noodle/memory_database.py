...
class NoodleMemoryDatabase(object):
        ...
        if original_db:
            ...
            for block in blocks:
                ...
                if block.public_key not in self.latest_blocks:
                    ...
                    self.latest_blocks[block.public_key].sequence_number
    ...
    def add_double_spend(self, block1, block2):
        ...
        """
        self.double_spends[(block1.public_key, block1.sequence_number)] = (
            block1,
            block2,
        )
    ...
    def add_block(self, block):
        ...
        if block.public_key not in self.latest_blocks:
            ...
            self.latest_blocks[block.public_key].sequence_number < block.sequence_number
        ):
            self.latest_blocks[block.public_key] = block
        ...
        self.block_time[(block.public_key, block.sequence_number)] = int(
    ...
    def add_spend(self, spend):
        ...
        if (
            id_from not in self.work_graph
            or id_to not in self.work_graph[id_from]
            or "total_spend" not in self.work_graph[id_from][id_to]
            or self.work_graph[id_from][id_to]["total_spend"]
            < float(spend.transaction["total_spend"])
        ):
            ...
            "spend_num" not in self.work_graph[id_from][id_to]
            or self.work_graph[id_from][id_to]["spend_num"] < spend.sequence_number
        ):
    ...
    def add_claim(self, claim):
        ...
        if (
            ...
            or "total_spend" not in self.work_graph[id_from][id_to]
            or self.work_graph[id_from][id_to]["total_spend"]
            < float(claim.transaction["total_spend"])
        ):
            self.work_graph.add_edge(
                id_from,
                id_to,
            ...
            "claim_num" not in self.work_graph[id_from][id_to]
            or self.work_graph[id_from][id_to]["claim_num"] < claim.sequence_number
        ):
    ...
    def update_chain_dependency(self, peer_id):
        if self.get_balance(peer_id, verified=True) >= 0:
            ...
            for k in self.work_graph.successors(peer_id):
                if (
                    "verified" in self.work_graph[peer_id][k]
                    and not self.work_graph[peer_id][k]["verified"]
                ):
    ...
    def get_last_pairwise_spend_num(self, peer_a, peer_b):
        if (
            not self.work_graph.has_edge(peer_a, peer_b)
    ...
    def get_total_spends(self, peer_id):
        if peer_id not in self.work_graph:
            ...
            return sum(
                self.work_graph[peer_id][k]["total_spend"]
                for k in self.work_graph.successors(peer_id)
            )

    def is_verified(self, p1, p2):
        return (
            "verified" in self.work_graph[p1][p2]
            and self.work_graph[p1][p2]["verified"]
        )
    ...
    def get_peer_status(self, public_key):
        ...
        for v in successors:
            status["spends"][v] = (
                self.get_total_pairwise_spends(peer_id, v),
        ...
        for v in predecessors:
            status["claims"][v] = (
                self.get_total_pairwise_spends(v, peer_id),
    ...
    def get_total_claims(self, peer_id, only_verified=True):
        if peer_id not in self.work_graph:
            ...
            return 0
        return sum(
            self.work_graph[k][peer_id]["total_spend"]
    ...
    def get_balance(self, peer_id, verified=True):
        ...
        return self.get_total_claims(
            peer_id, only_verified=verified
        ) - self.get_total_spends(peer_id)

    def remove_block(self, block):
        self.block_cache.pop((block.public_key, block.sequence_number), None)
        self.linked_block_cache.pop(
            (block.link_public_key, block.link_sequence_number), None
        )
    ...
    def get_last_pairwise_block(self, spender, claimer):
        ...
        if (
            not self.work_graph.has_edge(a_id, b_id)
            or "claim_num" not in self.work_graph[a_id][b_id]
        ):
            return None
        else:
    ...
    def get_lowest_range_unknown(self, public_key):
        ...
        known_block_nums = [
            seq_num for pk, seq_num in self.block_cache.keys() if pk == public_key
        ]
        filtered_block_nums = [
            seq_num for seq_num in known_block_nums if seq_num > lowest_unknown
        ]
                ...
                writer.writerow(
                    {
                        ...
                        "link": block.link_sequence_number,
                        "from_id": from_id,
                        "to_id": to_id,
                    }
                )
    ...
    def commit(self, my_pub_key):
        ...
        if self.original_db:
            my_blocks = [
                ...
                if block.public_key == my_pub_key
            ]
