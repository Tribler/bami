...
def task(func):
    ...
    def wrapper(self, *args, **kwargs):
        return self.register_anonymous_task(
            func.__name__,
...
class TaskManager(object):
    ...
    def __init__(self):
        ...
        self._checker = self.register_task(
            "_check_tasks",
            self._check_tasks,
        ...
        for name, task in self._pending_tasks.items():
            if not task.interval and now - task.start_time > MAX_TASK_AGE:
                self._logger.warning(
                    'Non-interval task "%s" has been running for %.2f!',
                    name,
                    now - task.start_time,
                )
    ...
    def register_task(self, name, task, *args, delay=None, interval=None, ignore=()):
        ...
        if (
            ...
            and not iscoroutinefunction(task)
            and not callable(task)
        ):
            raise ValueError(
    ...
    def register_anonymous_task(self, basename, task, *args, **kwargs):
        ...
        return self.register_task(
            basename + " " + str(self._counter), task, *args, **kwargs
        )
    ...
    def cancel_all_pending_tasks(self):
        ...
        with self._task_lock:
            assert all(
                [isinstance(t, (Task, Future)) for t in self._pending_tasks.values()]
            ), self._pending_tasks
            return [
