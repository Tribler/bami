...
from typing import Any, Dict, List, Optional, Set, Tuple, Type, Union, TYPE_CHECKING
import hashlib
import os
import time
...
from python_project.dht.payload import (
    BasePayload,
    FindRequestPayload,
    FindResponsePayload,
    PingRequestPayload,
...
from python_project.dht.routing import Node, RoutingTable, calc_node_id, distance
from python_project.dht.storage import Storage
from python_project.community import Community
from python_project.lazy_community import lazy_wrapper, lazy_wrapper_wd
...
if TYPE_CHECKING:
    ...
    from python_project.messaging.payload import (
...
async def gather_without_errors(*futures) -> Any:
...
class Request(RandomNumberCache):
    ...
    def __init__(
        self,
        community: Union[DHTCommunity, DHTDiscoveryCommunity],
        msg_type: str,
        node: Node,
        params: Optional[Union[List[bool], List[bytes]]] = None,
        ...
        if not self.future.done():
            ...
            if not self.consume_errors:
                self.future.set_exception(
                    DHTError("Timeout for {} to {}".format(self.msg_type, self.node))
                )
            else:
...
class DHTCommunity(Community):
    ...
    master_peer = Peer(
        unhexlify(
            "4c69624e61434c504b3a4c99f04cef9ba4ca645401cd51b8ef634e63e2ad0d3209eca958ce0293d7cf668"
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.logger.info(
            "DHT community initialized (peer mid %s)", hexlify(self.my_peer.mid)
        )

    def get_available_strategies(self) -> Dict[str, Type[PingChurn]]:
    ...
    def get_requesting_node(self, peer: Peer) -> Optional[Node]:
    ...
    def introduction_response_callback(
        self,
        peer: Peer,
        dist: GlobalTimeDistributionPayload,
    ...
    def on_node_discovered(
        self, public_key_bin: Union[LibNaCLSK, bytes], source_address: Tuple[str, int]
    ) -> None:
        # Filter out trackers
    ...
    def ping(self, node: Node) -> Future:
        ...
        cache = self.request_cache.add(
            Request(self, u"ping", node, consume_errors=True)
        )
    ...
    def on_ping_request(
        ...
        self.send_message(
            peer.address, MSG_PONG, PingResponsePayload, (payload.identifier,)
        )
    ...
    def serialize_value(self, data: bytes, sign: bool = True) -> bytes:
        if sign:
            payload = SignedStrPayload(
                data, int(time.time()), self.my_peer.public_key.key_to_bin()
            )
            return self._ez_pack(
                b"", DHT_ENTRY_STR_SIGNED, [payload.to_pack_list()], sig=True
            )
    ...
    def add_value(self, key: bytes, value: bytes, max_age: int = MAX_ENTRY_AGE) -> None:
    ...
    async def store_value(
        self, key: bytes, data: bytes, sign: bool = False
    ) -> List[Node]:
    ...
    @task
    async def store_on_nodes(
        self, key: bytes, values: List[bytes], nodes: List[Node]
    ) -> List[Node]:
        if not nodes:
        ...
        if (
            len(nodes) < TARGET_NODES
            or distance(self.my_node_id, key) < largest_distance
        ):
        ...
        for node in nodes:
            if (
                node in self.tokens
                and self.tokens[node][0] + TOKEN_EXPIRATION_TIME > now
            ):
                ...
                self.logger.debug(
                    "Not sending store-request to %s (no token available)", node
                )
    ...
    def on_store_request(
        ...
        for node in self.routing_table.closest_nodes(payload.target, max_nodes=20):
            if distance(node.id, payload.target) < distance(
                self.my_node_id, payload.target
            ):
                num_closer += 1
    ...
    def on_store_response(
        self, peer: Peer, dist: GlobalTimeDistributionPayload, payload: BasePayload
    ) -> None:
        if not self.request_cache.has(u"store", payload.identifier):
            self.logger.error(
                "Got store-response with unknown identifier, dropping packet"
            )
            return
    ...
    def _send_find_request(
        self, node: Node, target: bytes, force_nodes: bool, start_idx: int = 0
    ) -> Future:
    ...
    def _process_find_responses(
        self,
        responses: Union[
            List[Tuple[Node, Dict[str, List[Any]]]],
            List[Tuple[Node, Dict[str, List[Node]]]],
            List[Tuple[Node, Dict[str, List[bytes]]]],
            List[
                Union[
                    Tuple[Node, Dict[str, List[bytes]]],
                    Tuple[Node, Dict[str, List[Node]]],
                ]
            ],
        ],
        nodes_tried: Set[Node],
    ) -> Union[
        Tuple[List[Any], Set[Node], Dict[Node, Node]],
        Tuple[List[Any], Set[Any], Dict[Any, Any]],
        Tuple[List[Any], Set[Node], Dict[Any, Any]],
        Tuple[List[List[bytes]], Set[Node], Dict[Node, Node]],
        Tuple[List[List[bytes]], Set[Any], Dict[Any, Any]],
    ]:
        values = []
        nodes = set()
        to_puncture = {}
            ...
            if "values" in response:
                ...
                node = next(
                    (
                        ...
                        if n not in nodes_tried and n not in list(to_puncture.values())
                    ),
                    None,
                )
    ...
    async def _find(
        self, target: bytes, force_nodes: bool = False, start_idx: int = 0
    ) -> Union[
        List[Node],
        List[Tuple[bytes, None]],
        List[Tuple[bytes, bytes]],
        List[Union[Tuple[bytes, bytes], Tuple[bytes, None]]],
    ]:
        ...
        for _ in range(MAX_FIND_STEPS):
            ...
            futures = [
                self._send_find_request(node, target, force_nodes, start_idx=start_idx)
                for node in nodes_closest
            ]
            responses = await gather_without_errors(*futures)
            recent = next(
                (sender for sender, response in responses if "nodes" in response),
                recent,
            )
            ...
            if len(nodes_closest) > MAX_FIND_WALKS:
                nodes_closest = set(
                    sorted(nodes_closest, key=lambda n: distance(n.id, target))[
                        :MAX_FIND_WALKS
                    ]
                )
    ...
    def on_find_request(
        ...
        peer: Peer,
        dist: GlobalTimeDistributionPayload,
        payload: FindRequestPayload,
    ) -> None:
        self.logger.debug(
            "Got find-request for %s from %s", hexlify(payload.target), peer.address
        )
        ...
        values = (
            self.storage.get(
                payload.target,
                starting_point=payload.start_idx,
                limit=MAX_VALUES_IN_FIND,
            )
            if not payload.force_nodes
            else []
        )

        if payload.force_nodes or not values:
            nodes = self.routing_table.closest_nodes(
                payload.target, exclude_node=node, max_nodes=MAX_NODES_IN_FIND
            )
            # Send puncture request to the closest node
            if nodes:
                packet = self.create_puncture_request(
                    payload.lan_address, peer.address, payload.identifier
                )
    ...
    def on_find_response(
        ...
        if not self.request_cache.has(u"find", payload.identifier):
            self.logger.error(
                "Got find-response with unknown identifier, dropping packet"
            )
            return
    ...
    async def value_maintenance(self) -> None:
        # Refresh buckets
        now = time.time()
    ...
    def check_token(self, node: Node, token: bytes) -> bool:
        return any(
            [
                hashlib.sha1(cast_to_bin(str(node)) + secret).digest() == token
