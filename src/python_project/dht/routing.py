...
from python_project.dht.trie import Trie
from python_project.peer import Peer
from python_project.util import cast_to_bin, cast_to_unicode
...
def distance(a, b):
    return int(binascii.hexlify(cast_to_bin(a)), 16) ^ int(
        binascii.hexlify(cast_to_bin(b)), 16
    )
...
def calc_node_id(ip, mid):
    ...
    ip_masked = "".join(
        [chr(ord(ip_bin[i : i + 1]) & ord(ip_mask[i : i + 1])) for i in range(4)]
    )
...
class Node(Peer):
    ...
    def status(self):
        ...
        elif ((now - self.last_response) < 15 * 60) or (
...
class Bucket(object):
    ...
    def generate_id(self):
        rand_node_id_bin = format(
            random.randint(0, 2 ** (160 - len(self.prefix_id))), "0160b"
        )
...
class RoutingTable(object):
    ...
    def get_bucket(self, node_id):
        node_id_binary = id_to_binary_string(node_id)
        return (
            self.trie.longest_prefix_value(
                cast_to_unicode(node_id_binary), default=None
            )
            or self.trie[u""]
        )
    ...
    def closest_nodes(self, node_id, max_nodes=8, exclude_node=None):
        with self.lock:
            ...
            return sorted(nodes, key=lambda n: (distance(n.id, node_id), n.status))[
                :max_nodes
            ]
