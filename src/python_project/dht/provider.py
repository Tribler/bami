...
class DHTCommunityProvider(object):
    ...
    async def peer_lookup(self, mid):
        try:
            ...
            self.logger.debug(
                "Failed to connect %s using the DHTCommunity (error: %s)",
                hexlify(mid),
                e,
            )
            return
    ...
    async def lookup(self, info_hash):
        try:
            ...
            self.logger.info(
                "Failed to lookup %s on the DHTCommunity (error: %s)",
                hexlify(info_hash),
                e,
            )
            return
        ...
        for value, _ in values:
            try:
                ...
                seeder_pk = (
                    b"LibNaCLPK:"
                    + value[14 + intro_key_len : 14 + intro_key_len + seeder_key_len]
                )

                results.append(
                    IntroductionPoint(intro_peer, seeder_pk, PEER_SOURCE_DHT, last_seen)
                )
            except Exception as e:
                self.logger.info(
                    "Error encountered during lookup %s on the DHTCommunity (error: %s)",
                    hexlify(info_hash),
                    e,
                )
        self.logger.info(
            "Looked up %s in the DHTCommunity, got %d results",
            hexlify(info_hash),
            len(results),
        )
    ...
    async def announce(self, info_hash, intro_point):
        ...
        value = inet_aton(intro_point.peer.address[0]) + pack(
            "!H", intro_point.peer.address[1]
        )
        ...
        try:
            ...
            self.logger.info(
                "Failed to announce %s to the DHTCommunity (error: %s)",
                hexlify(info_hash),
                e,
            )
        else:
