...
from python_project.dht.community import (
    DHTCommunity,
    MAX_NODES_IN_FIND,
    PING_INTERVAL,
    Request,
    TARGET_NODES,
    gather_without_errors,
)
...
from python_project.dht.routing import NODE_STATUS_BAD, Node
from python_project.lazy_community import lazy_wrapper, lazy_wrapper_wd
from python_project.messaging.payload_headers import GlobalTimeDistributionPayload
...
class DHTDiscoveryCommunity(DHTCommunity):
    ...
    async def send_store_peer_request(self, key, nodes):
        ...
        nodes = [
            Node(node.key, node.address)
            for node in nodes
            if node not in self.store_for_me[key]
        ]
        ...
        for node in nodes:
            if node in self.tokens:
                cache = self.request_cache.add(
                    Request(self, u"store-peer", node, [key])
                )
                futures.append(cache.future)
                self.send_message(
                    node.address,
                    MSG_STORE_PEER_REQUEST,
                    StorePeerRequestPayload,
                    (cache.number, self.tokens[node][1], key),
                )
            else:
                self.logger.debug(
                    "Not sending store-peer-request to %s (no token available)", node
                )
    ...
    async def send_connect_peer_request(self, key, nodes):
        ...
        for node in nodes:
            ...
            self.send_message(
                node.address,
                MSG_CONNECT_PEER_REQUEST,
                ConnectPeerRequestPayload,
                (cache.number, self.my_estimated_lan, key),
            )
    ...
    def on_store_peer_request(self, peer, dist, payload):
        ...
        if payload.target != peer.mid:
            self.logger.warning(
                "Not allowed to store under key %s, dropping packet.",
                hexlify(payload.target),
            )
            return
    ...
    def on_store_peer_response(self, peer, dist, payload):
        if not self.request_cache.has(u"store-peer", payload.identifier):
            self.logger.error(
                "Got store-peer-response with unknown identifier, dropping packet"
            )
            return
    ...
    def on_connect_peer_request(self, peer, dist, payload):
        ...
        self.send_message(
            peer.address,
            MSG_CONNECT_PEER_RESPONSE,
            ConnectPeerResponsePayload,
            (payload.identifier, nodes),
        )
    ...
    def on_connect_peer_response(self, peer, dist, payload):
        if not self.request_cache.has(u"connect-peer", payload.identifier):
            self.logger.error(
                "Got connect-peer-response with unknown identifier, dropping packet"
            )
            return
        ...
        for key, nodes in self.store_for_me.items():
            for index in range(len(nodes) - 1, -1, -1):
                ...
                if node.status == NODE_STATUS_BAD:
                    ...
                    self.logger.debug(
                        "Deleting peer %s that stored us (key %s)", node, hexlify(key)
                    )
