...
from python_project.keyvault.keys import Key
...
_CURVES = {
    u"very-low": (ec.SECT163K1, "M2Crypto"),
...
class ECCrypto(object):
    ...
    def generate_key(self, security_level: str) -> Union[M2CryptoSK, LibNaCLSK]:
        """
    ...
    def key_to_bin(self, ec: Union[M2CryptoSK, LibNaCLPK, LibNaCLSK]) -> bytes:
        "Convert the key to a binary format."
        assert isinstance(ec, Key), ec
        return ec.key_to_bin()

    def key_to_hash(self, ec: Union[M2CryptoSK, LibNaCLSK]) -> bytes:
    ...
    def key_from_private_bin(self, string: bytes) -> Union[M2CryptoSK, LibNaCLSK]:
    ...
    def key_from_public_bin(self, string: bytes) -> Union[M2CryptoPK, LibNaCLPK]:
    ...
    def get_signature_length(self, ec: Union[M2CryptoPK, LibNaCLPK]) -> int:
        """
    ...
    def create_signature(self, ec: Union[M2CryptoSK, LibNaCLSK], data: bytes) -> bytes:
        """
    ...
    def is_valid_signature(
        self,
        ec: Union[M2CryptoPK, LibNaCLPK, LibNaCLSK],
        data: bytes,
