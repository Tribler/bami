...
class TrustChainCommunity(Community):
    ...
    master_peer = Peer(
        unhexlify(
            "4c69624e61434c504b3a5730f52156615ecbcedb36c442992ea8d3c26b418edd8bd00e01dce26028cd"
    ...
    def sign_block(
        ...
        assert (
            ...
            or additional_info is not None
            and linked is not None
            and transaction is None
            and peer == self.my_peer
            and public_key == linked.public_key
        ...
        self.logger.info(
            "Signed block to %s (%s) validation result %s",
            hexlify(block.link_public_key)[-8:],
            block,
            validation,
        )
        if (
            validation[0] != ValidationResult.partial_next
            and validation[0] != ValidationResult.valid
        ):
            self.logger.error(
                "Signed block did not validate?! Result %s", repr(validation)
            )
    ...
    async def process_half_block(self, blk, peer):
        ...
        self.logger.info(
            "Block validation result %s, %s, (%s)", validation[0], validation[1], blk
        )
        if validation[0] == ValidationResult.invalid:
            raise RuntimeError(
        ...
        if (
            validation[0] == ValidationResult.partial_previous
            ...
            if not self.request_cache.has("crawl", blk.hash_number):
                try:
                    await self.send_crawl_request(
                        ...
                        max(
                            GENESIS_SEQ,
                            (blk.sequence_number - self.settings.validation_range),
                        ),
    ...
    def create_introduction_request(self, socket_address, extra_bytes=b""):
        ...
        return super(TrustChainCommunity, self).create_introduction_request(
...
class TrustChainTestnetCommunity(TrustChainCommunity):
    ...
    master_peer = Peer(
        unhexlify(
            "4c69624e61434c504b3aa90c1e65d68e9f0ccac1385b58e4a605add2406aff9952b1b6435ab07e5385"
