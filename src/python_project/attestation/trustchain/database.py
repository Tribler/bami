...
from python_project.attestation.trustchain.blockcache import BlockCache
from python_project.database import Database, database_blob
...
class TrustChainDB(Database):
    ...
    def add_block(self, block):
        ...
        if self.my_blocks_cache and (
            block.public_key == self.my_pk or block.link_public_key == self.my_pk
        ):
    ...
    def _get(self, query, params):
        db_result = list(
            self.execute(self.get_sql_header() + query, params, fetch_all=False)
        )
        db_block_class = None
        if db_result:
            db_block_class = (
                db_result[0]
                if isinstance(db_result[0], bytes)
                else str(db_result[0]).encode("utf-8")
            )
    ...
    def get(self, public_key, sequence_number):
        ...
        """
        return self._get(
            u"WHERE public_key = ? AND sequence_number = ?",
    ...
    def get_number_of_known_blocks(self, public_key=None):
        ...
        if public_key:
            return list(
                self.execute(
                    u"SELECT COUNT(*) FROM blocks WHERE public_key = ?",
                    (database_blob(public_key),),
                )
            )[0][0]
    ...
    def get_blocks_with_type(self, block_type, public_key=None):
        ...
        if public_key:
            return self._getall(
                u"WHERE type = ? and public_key = ?",
                (block_type, database_blob(public_key)),
            )
    ...
    def get_latest(self, public_key, block_type=None):
        ...
        if block_type:
            return self._get(
                u"WHERE public_key = ? AND type = ? AND sequence_number = (SELECT MAX(sequence_number) "
            ...
            return self._get(
                u"WHERE public_key = ? AND sequence_number = (SELECT MAX(sequence_number) FROM blocks "
                u"WHERE public_key = ?)",
                (database_blob(public_key), database_blob(public_key)),
            )
    ...
    def get_latest_blocks(self, public_key, limit=25, block_types=None):
        ...
        if block_types:
            return self._getall(
                u"WHERE public_key = ? AND type IN (?) ORDER BY sequence_number DESC LIMIT ?",
                (database_blob(public_key), b",".join(block_types), limit),
            )
        else:
            return self._getall(
                u"WHERE public_key = ? ORDER BY sequence_number DESC LIMIT ?",
    ...
    def get_block_after(self, block, block_type=None):
        ...
        if block_type:
            return self._get(
                u"WHERE sequence_number > ? AND public_key = ? AND type = ? ORDER BY sequence_number ASC",
                (block.sequence_number, database_blob(block.public_key), block_type),
            )
        else:
            return self._get(
                u"WHERE sequence_number > ? AND public_key = ? ORDER BY sequence_number ASC",
    ...
    def get_block_before(self, block, block_type=None):
        ...
        if block_type:
            return self._get(
                u"WHERE sequence_number < ? AND public_key = ? AND type = ? ORDER BY sequence_number DESC",
                (block.sequence_number, database_blob(block.public_key), block_type),
            )
        else:
            return self._get(
                u"WHERE sequence_number < ? AND public_key = ? ORDER BY sequence_number DESC",
                (block.sequence_number, database_blob(block.public_key)),
            )
    ...
    def get_lowest_sequence_number_unknown(self, public_key):
        ...
        query = (
            u"SELECT b1.sequence_number FROM blocks b1 WHERE b1.public_key = ? AND NOT EXISTS "
    ...
    def get_lowest_range_unknown(self, public_key):
        ...
        query = (
            u"SELECT sequence_number FROM blocks WHERE public_key = ? AND sequence_number > ? "
            u"ORDER BY sequence_number LIMIT 1"
        )
        db_result = list(
            self.execute(
                query, (database_blob(public_key), lowest_unknown), fetch_all=True
            )
        )
        if db_result:
    ...
    def get_linked(self, block):
        ...
        """
        return self._get(
            u"WHERE public_key = ? AND sequence_number = ? OR link_public_key = ? AND "
            u"link_sequence_number = ? ORDER BY block_timestamp ASC",
            (
                database_blob(block.link_public_key),
                block.link_sequence_number,
                database_blob(block.public_key),
    ...
    def crawl(self, public_key, start_seq_num, end_seq_num, limit=100):
        if self.my_blocks_cache and public_key == self.my_pk:
            ...
            query = (
                u"SELECT * FROM (%s WHERE sequence_number >= ? AND sequence_number <= ? AND public_key = ? "
    ...
    def get_recent_blocks(self, limit=10, offset=0):
        ...
        """
        return self._getall(
            u"ORDER BY block_timestamp DESC LIMIT ? OFFSET ?", (limit, offset)
        )
    ...
    def get_users(self, limit=100):
        ...
        res = list(
            self.execute(
                ...
                u"ORDER BY MAX(sequence_number) DESC LIMIT ? ",
                (limit,),
            )
        )
        users_info = []
            ...
            users_info.append(
                {
                    "public_key": hexlify(
                        user_info[0]
                        if isinstance(user_info[0], bytes)
                        else str(user_info[0])
                    ),
                    "blocks": user_info[1],
                }
            )
    ...
    def add_double_spend(self, block1, block2):
        ...
        """
        sql = (
            u"INSERT OR IGNORE INTO double_spends (type, tx, public_key, sequence_number, link_public_key,"
            u"link_sequence_number,previous_hash, signature, block_timestamp, block_hash) VALUES(?,?,?,?,?,?,?,?,?,?)"
        )
    ...
    def did_double_spend(self, public_key):
        ...
        count = list(
            self.execute(
                u"SELECT COUNT(*) FROM double_spends WHERE public_key = ?",
                (database_blob(public_key),),
            )
        )[0][0]
        return count > 0
    ...
    def get_sql_header(self):
        ...
        """
        _columns = (
            u"type, tx, public_key, sequence_number, link_public_key, link_sequence_number, "
    ...
    def get_schema(self):
        ...
        """ % (
            ...
            self.get_sql_create_blocks_table(
                "double_spends", "public_key, sequence_number, block_hash"
            ),
    ...
    def get_upgrade_script(self, current_version):
        ...
        if current_version <= 4 or current_version == 6:
            ...
            return self.get_sql_create_blocks_table(
                "double_spends", "public_key, sequence_number, block_hash"
            )
    ...
    def check_database(self, database_version):
        ...
        if database_version < self.LATEST_DB_VERSION:
            if (
                database_version > 0
            ):  # Only run the upgrade loop if there is something to upgrade.
                while database_version < self.LATEST_DB_VERSION:
                    upgrade_script = self.get_upgrade_script(
