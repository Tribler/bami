...
class ProofU(object):
    ...
    def Verify(self, pk, context, nonce):
        return self.VerifyWithChallenge(
            pk, createChallenge(context, nonce, self.ChallengeContribution(pk), False)
        )
...
class ProofD(object):
    ...
    def Verify(self, pk, context, nonce1, issig):
        return self.VerifyWithChallenge(
            pk, createChallenge(context, nonce1, self.ChallengeContribution(pk), issig)
        )
    ...
    def Copy(self):
        ...
        for k, v in self.ADisclosed.items():
            ADisclosed[k] = v
        return ProofD(
            self.C, self.A, self.EResponse, self.VResponse, self.AResponses, ADisclosed
        )
