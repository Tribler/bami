...
def page_loaded(ok):
    if ok and my_web.url() == QUrl(u"https://services.nijmegen.nl/irma/issue?"):
        my_web.page().runJavaScript(
            'document.getElementById("qrcode").title', print_result
        )
...
response = do_post(
    u,
    '[{"issuer":{"identifier":"pbdf.nijmegen"},"counter": 0},{"issuer":{"identifier":"pbdf.nijmegen"},"counter": 0},{"issuer":{"identifier":"pbdf.nijmegen"},"counter": 0},{"issuer":{"identifier":"pbdf.nijmegen"},"counter": 0}]',
...
u = (
    "https://www.nijmegen.nl/personen/attributen/irma-server-api/api/v2/issue/%s"
    % token
)
...
u = (
    "https://www.nijmegen.nl/personen/attributen/irma-server-api/api/v2/issue/%s/status"
    % token
)
...
u = (
    "https://www.nijmegen.nl/personen/attributen/irma-server-api/api/v2/issue/%s/commitments"
    % token
)
...
outjson += '"U": null,'
outjson += (
    '"combinedProofs": [' + ", ".join(proof_to_str(p) for p in commitMsg.Proofs) + "],"
)
outjson += '"indices": [],'
...
for ism in output_proof:
    ...
    proof_s = ProofS(
        b64_to_int(proof_s_desc[u"c"]), b64_to_int(proof_s_desc[u"e_response"])
    )
    signature = CLSignature(
        b64_to_int(sig_desc[u"A"]),
        b64_to_int(sig_desc[u"e"]),
        b64_to_int(sig_desc[u"v"]),
    )
    isms.append(IssueSignatureMessage(signature, proof_s))


order_map = {
    u"pbdf.nijmegen.address": [
        u"street",
        u"houseNumber",
        u"zipcode",
        u"municipality",
...
for i in range(len(cbs)):
    ...
    attribute_ints, signing_date = make_attribute_list(
        issuance_output[u"credentials"][i], ordering
    )

    credential = cb.ConstructCredential(ism, attribute_ints)
    builder = credential.CreateDisclosureProofBuilder(
        list(range(1, len(attribute_ints) + 1))
    )
