...
def generate_safe_prime(bit_length: int, backend: Backend = default_backend()) -> int:
    """
    ...
    err = backend._lib.BN_generate_prime_ex(
        generated,
        bit_length,
        1,
        backend._ffi.NULL,
...
def is_prime(number: int, backend: Backend = default_backend()) -> bool:
    """
    ...
    result = backend._lib.BN_is_prime_ex(
        generated,
        backend._lib.BN_prime_checks_for_size(int(len(hex_n) * 8)),
        backend._ffi.NULL,
        backend._ffi.NULL,
    )
    backend._lib.BN_clear_free(generated)
