...
class AttestationsDB(Database):
    ...
    def get_attestation_by_hash(self, attestation_hash):
        return self._get(
            u"SELECT blob FROM %s WHERE hash = ?" % self.db_name,
            (database_blob(attestation_hash),),
        )

    def get_all(self):
        return list(
            self.execute(u"SELECT * FROM %s" % self.db_name, (), fetch_all=True)
        )
    ...
    def insert_attestation(self, attestation, attestation_hash, secret_key, id_format):
        ...
        self.execute(
            u"INSERT INTO %s (hash, blob, key, id_format) VALUES(?,?,?,?)"
            % self.db_name,
            (
                database_blob(attestation_hash),
                blob,
                database_blob(secret_key.serialize()),
