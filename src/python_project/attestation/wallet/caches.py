...
class ReceiveAttestationVerifyCache(HashCache):
    ...
    def __init__(self, community, cache_hash, id_format):
        super(ReceiveAttestationVerifyCache, self).__init__(
            community.request_cache,
            u"receive-verify-attestation",
            cache_hash,
            id_format,
        )
...
class ReceiveAttestationRequestCache(PeerCache):
    ...
    def __init__(self, community, mid, key, name, id_format):
        super(ReceiveAttestationRequestCache, self).__init__(
            community.request_cache, u"receive-request-attestation", mid, id_format
        )
        self.attestation_map = set()
        self.key = key
        self.name = name
...
class ProvingAttestationCache(HashCache):
    ...
    def __init__(
        ...
        id_format,
        public_key=None,
        on_complete=lambda x, y: None,
    ):
        super(ProvingAttestationCache, self).__init__(
            community.request_cache, u"proving-attestation", cache_hash, id_format
        )
...
class PendingChallengeCache(HashCache):
    ...
    def __init__(
        self, community, cache_hash, proving_cache, id_format, honesty_check=-1
    ):
        super(PendingChallengeCache, self).__init__(
            community.request_cache, u"proving-hash", cache_hash, id_format
        )
