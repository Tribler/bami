...
from python_project.attestation.wallet.bonehexact.structs import (
    BitPairAttestation,
    BonehAttestation,
)
...
def attest(
    PK: Union[BonehPublicKey, BonehPrivateKey], value: int, bitspace: int
) -> BonehAttestation:
    """
    ...
    for i in range(0, len(A) - 1, 2):
        t_out_private.append(
            (i, encode(PK, PK.p - ((R[i] + R[i + 1]) % (PK.p + 1)) + 1))
        )
    # Shuffle:
    t_out_public = [
        (i, t_out_public[i], t_out_public[i + 1])
        for i in range(0, len(t_out_public), 2)
    ]
...
def binary_relativity_certainty(
    expected: Dict[int, int], value: Dict[int, int]
) -> float:
    """
...
def create_challenge_response_from_pair(
    SK: BonehPrivateKey, pair: Tuple[int, int, bytes]
) -> int:
    """
