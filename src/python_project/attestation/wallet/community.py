...
from ...messaging.payload_headers import (
...
class AttestationCommunity(Community):
    ...
    master_peer = Peer(
        unhexlify(
            "3081a7301006072a8648ce3d020106052b810400270381920004057a009787f66ea54d5082ea2f56a8"
    ...
    def __init__(self, *args, **kwargs):
        ...
        for attribute_hash, _, key, id_format in self.database.get_all():
            attribute_hash = (
                attribute_hash
                if isinstance(attribute_hash, bytes)
                else str(attribute_hash)
            )
            key = key if isinstance(key, bytes) else str(key)
            id_format = (
                id_format if isinstance(id_format, bytes) else str(id_format)
            ).decode("utf-8")
            self.attestation_keys[attribute_hash] = (
                self.get_id_algorithm(id_format).load_secret_key(key),
                id_format,
            )
    ...
    def dump_blob(self, attribute_name, id_format, blob, metadata={}):
        ...
        attestation = id_algorithm.get_attestation_class().unserialize_private(
            key, attestation_blob, id_format
        )
    ...
    def request_attestation(self, peer, attribute_name, secret_key, metadata={}):
        ...
        metadata = cast_to_bin(
            json.dumps(
                {cast_to_unicode(k): cast_to_unicode(v) for k, v in meta_dict.items()}
            )
        )
        ...
        self.request_cache.add(
            ReceiveAttestationRequestCache(
                self, peer.mid + gtime_str, secret_key, attribute_name, id_format
            )
        )
        self.allowed_attestations[peer.mid] = self.allowed_attestations.get(
            peer.mid, []
        ) + [gtime_str]
    ...
    async def on_request_attestation(self, peer, dist, payload):
        ...
        value = await maybe_coroutine(
            self.attestation_request_callback, peer, attribute, metadata
        )
        if value is None:
            return
        ...
        attestation = id_algorithm.get_attestation_class().unserialize(
            attestation_blob, id_format
        )

        self.attestation_request_complete_callback(
            peer, attribute, attestation.get_hash(), id_format
        )

        self.send_attestation(peer.address, attestation_blob, dist.global_time)

    def on_attestation_complete(
        self, unserialized, secret_key, peer, name, attestation_hash, id_format
    ):
        """
        We got an Attestation delivered to us.
        """
        self.attestation_keys[cast_to_bin(attestation_hash)] = (secret_key, id_format)
        self.database.insert_attestation(
            unserialized, attestation_hash, secret_key, id_format
        )
        self.attestation_request_complete_callback(
            self.my_peer, name, attestation_hash, id_format, peer
        )

    def verify_attestation_values(
        self, socket_address, attestation_hash, values, callback, id_format
    ):
        """
        ...
        def on_complete(attestation_hash, relativity_map):
            callback(
                attestation_hash,
                [algorithm.certainty(value, relativity_map) for value in values],
            )

        self.request_cache.add(
            ProvingAttestationCache(
                self, attestation_hash, id_format, on_complete=on_complete
            )
        )
        self.create_verify_attestation_request(
            socket_address, attestation_hash, id_format
        )

    def create_verify_attestation_request(
        self, socket_address, attestation_hash, id_format
    ):
        """
        Ask the peer behind a socket address to deliver the Attestation with a certain hash.
        ...
        """
        self.request_cache.add(
            ReceiveAttestationVerifyCache(self, attestation_hash, id_format)
        )
    ...
    async def on_verify_attestation_request(self, peer, dist, payload):
        ...
        private_attestation = attestation_cls.unserialize_private(
            SK, attestation_blob, id_format
        )
    ...
    def send_attestation(self, socket_address, blob, global_time=None):
        ...
        for i in range(0, len(blob), 800):
            ...
            self.logger.debug(
                "Sending attestation chunk %d to %s",
                sequence_number,
                str(socket_address),
            )
            ...
            payload = AttestationChunkPayload(
                sha1(blob).digest(), sequence_number, blob_chunk
            ).to_pack_list()
            dist = GlobalTimeDistributionPayload(global_time).to_pack_list()
    ...
    def on_attestation_chunk(self, peer, dist, payload):
        ...
        peer_ids = [
            PeerCache.id_from_address(
                u"receive-request-attestation", peer.mid + allowed_glob
            )
        ...
        if self.request_cache.has(*hash_id):
            ...
            cache.attestation_map |= {
                (payload.sequence_number, payload.data),
            }
            ...
            attestation_class = self.get_id_algorithm(
                cache.id_format
            ).get_attestation_class()
            if sha1(serialized).digest() == payload.hash:
                unserialized = attestation_class.unserialize(
                    serialized, cache.id_format
                )
                self.request_cache.pop(*hash_id)
                self.on_received_attestation(peer, unserialized, payload.hash)

            self.logger.debug(
                "Received attestation chunk %d for proving by %s",
            ...
            for peer_id in peer_ids:
                if self.request_cache.has(*peer_id):
                    ...
                    for (_, chunk) in sorted(
                        cache.attestation_map, key=lambda item: item[0]
                    ):
                        serialized += chunk
                    ...
                    if sha1(serialized).digest() == payload.hash:
                        unserialized = attestation_class.unserialize_private(
                            cache.key, serialized, cache.id_format
                        )
                        cache = self.request_cache.pop(*peer_id)
                        self.allowed_attestations[peer.mid] = [
                            glob_time
                            for glob_time in self.allowed_attestations[peer.mid]
                        ...
                        self.on_attestation_complete(
                            ...
                            peer,
                            cache.name,
                            unserialized.get_hash(),
                            cache.id_format,
                        )

                    self.logger.debug(
                        "Received attestation chunk %d for my attribute %s",
                        payload.sequence_number,
                        cache.name,
                    )
                    handled = True
            ...
            if not handled:
                self.logger.warning(
                    "Received Attestation chunk which we did not request!"
                )
    ...
    def on_received_attestation(self, peer, attestation, attestation_hash):
        ...
        cache = self.request_cache.get(
            *HashCache.id_from_hash(u"proving-attestation", attestation_hash)
        )
        ...
        for challenge in challenges:
            if remaining == 0:
                break
            elif self.request_cache.has(
                *PendingChallengeCache.id_from_hash(
                    u"proving-hash", sha1(challenge).digest()
                )
            ):
                continue
            remaining -= 1
            self.request_cache.add(
                PendingChallengeCache(
                    self, sha1(challenge).digest(), cache, cache.id_format
                )
            )
    ...
    def on_challenge_response(self, peer, dist, payload):
        ...
        """
        cache = self.request_cache.get(
            *HashCache.id_from_hash(u"proving-hash", payload.challenge_hash)
        )
        if cache:
            ...
            pcache_prefix, pcache_id = HashCache.id_from_hash(
                u"proving-attestation", proving_cache.hash
            )
            challenge = None
            ...
            if cache.honesty_check < 0:
                algorithm.process_challenge_response(
                    proving_cache.relativity_map, challenge, payload.response
                )
            elif not algorithm.process_honesty_challenge(
                cache.honesty_check, payload.response
            ):
                ...
                proving_cache.attestation_callbacks(
                    proving_cache.hash, algorithm.create_certainty_aggregate(None)
                )
            if len(proving_cache.hashed_challenges) == 0:
                ...
                if self.request_cache.has(pcache_prefix, pcache_id):
                    self.request_cache.pop(pcache_prefix, pcache_id)
                proving_cache.attestation_callbacks(
                    proving_cache.hash, proving_cache.relativity_map
                )
            else:
                # Send another proving hash
                honesty_check = algorithm.honesty_check and (
                    ord(os.urandom(1)[0:1]) < 38
                )
                honesty_check_byte = choice([0, 1, 2]) if honesty_check else -1
                challenge = None
                if honesty_check:
                    while not challenge or self.request_cache.has(
                        *HashCache.id_from_hash(
                            u"proving-hash", sha1(challenge).digest()
                        )
                    ):
                        challenge = algorithm.create_honesty_challenge(
                            proving_cache.public_key, honesty_check_byte
                        )
                if (not honesty_check) or (
                    challenge
                    and self.request_cache.has(
                        *HashCache.id_from_hash(
                            u"proving-hash", sha1(challenge).digest()
                        )
                    )
                    ...
                    for c in proving_cache.challenges:
                        if not self.request_cache.has(
                            *HashCache.id_from_hash(u"proving-hash", sha1(c).digest())
                        ):
                            challenge = c
                            break
                    if not challenge:
