...
class EL(object):
    def __init__(self, c: int, D: int, D1: int, D2: int) -> None:
        self.c = c
        self.D = D
        self.D1 = D1
        self.D2 = D2
    ...
    def create(
        ...
        c = sha256_as_int(
            str(cW1.a).encode("utf-8")
            + str(cW1.b).encode("utf-8")
            + str(cW2.a).encode("utf-8")
            + str(cW2.b).encode("utf-8")
        )
        D = w + c * x
        D1 = n1 + c * r1
        D2 = n2 + c * r2
    ...
    @classmethod
    def unserialize(cls, s: bytes) -> Tuple[EL, bytes]:
        unpacked, rem = _siunpack(s, 4)
        return cls(*unpacked), rem

    def __eq__(self, other: EL) -> bool:
...
class SQR(object):
    def __init__(self, F: FP2Value, el: EL) -> None:
        self.F = F
        self.el = el

    @classmethod
    def create(
        cls, x: int, r1: int, g: FP2Value, h: FP2Value, b: int, bitspace: int
    ) -> SQR:
    ...
    @classmethod
    def unserialize(cls, s: bytes) -> Tuple[SQR, bytes]:
        rem = s
        mod, rem = iunpack(rem)
        Fa, rem = iunpack(rem)
        Fb, rem = iunpack(rem)
    ...
    def __eq__(self, other: SQR) -> bool:
