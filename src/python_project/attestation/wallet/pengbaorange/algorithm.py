...
class PengBaoRangeAlgorithm(IdentityAlgorithm):
    ...
    def attest(self, PK, value):
        ...
        return create_attest_pair(
            PK, ivalue, self.a, self.b, self.key_size
        ).serialize_private(PK)
    ...
    def create_challenges(self, PK, attestation):
        ...
        challenges = [
            pack_pair(
                _safe_rndint(self.key_size, mod), _safe_rndint(self.key_size, mod)
            )
            for _ in range(1)
        ]
        return challenges
    ...
    def create_challenge_response(self, SK, attestation, challenge):
        ...
        if s < LARGE_INTEGER or t < LARGE_INTEGER:
            return pack_pair(
                _safe_rndint(self.key_size, SK.g.mod),
                _safe_rndint(self.key_size, SK.g.mod),
            ) + pack_pair(
                _safe_rndint(self.key_size, SK.g.mod),
                _safe_rndint(self.key_size, SK.g.mod),
    ...
    def process_challenge_response(self, aggregate, challenge, response):
        ...
        aggregate[challenge] = attestation.publicdata.check(
            self.a, self.b, s, t, x, y, u, v
        )
        return aggregate
