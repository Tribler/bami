...
class PengBaoCommitment(object):
    ...
    def serialize(self):
        return (
            ipack(self.c.mod)
            + _serialize_fp2value(self.c)
            + _serialize_fp2value(self.c1)
            + _serialize_fp2value(self.c2)
            + _serialize_fp2value(self.ca)
            + _serialize_fp2value(self.ca1)
            + _serialize_fp2value(self.ca2)
            + _serialize_fp2value(self.ca3)
            + _serialize_fp2value(self.caa)
        )
...
class PengBaoPublicData(object):
    ...
    def check(self, a, b, s, t, x, y, u, v):  # pylint: disable=R0913
        ...
        out &= (
            self.commitment.caa
            == self.commitment.ca1 * self.commitment.ca2 * self.commitment.ca3
        )
        out &= (self.PK.g.intpow(x) * self.PK.h.intpow(u)) == (
            self.commitment.ca1.intpow(s) * self.commitment.ca2 * self.commitment.ca3
        )
        out &= (self.PK.g.intpow(y) * self.PK.h.intpow(v)) == (
            self.commitment.ca1 * self.commitment.ca2.intpow(t) * self.commitment.ca3
        )
        out &= x > 0
        out &= y > 0
        return out
