...
from python_project.requestcache import NumberCache, RandomNumberCache
from python_project.util import maximum_integer
...
class NoodleCrawlRequestCache(NumberCache):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(NoodleCrawlRequestCache, self).__init__(
            community.request_cache, u"noodle-crawl", crawl_id
        )
        self.logger = logging.getLogger(self.__class__.__name__)
        self.community = community
        self.crawl_future = crawl_future
        self.received_half_blocks = []
        self.total_half_blocks_expected = (
            total_blocks if total_blocks else maximum_integer
        )
...
class AuditRequestCache(NumberCache):
    ...
    def __init__(self, community, crawl_id, audit_future, total_expected_audits):
        super(AuditRequestCache, self).__init__(
            community.request_cache, self.CACHE_IDENTIFIER, crawl_id
        )
        self.logger = logging.getLogger(self.__class__.__name__)
        self.community = community
    ...
    def on_timeout(self):
        self._logger.info(
            "Timeout for audit with id %d (received proofs: %d)",
            self.number,
...
class AuditProofRequestCache(NumberCache):
    ...
    def __init__(self, community, crawl_id):
        super(AuditProofRequestCache, self).__init__(
            community.request_cache, self.CACHE_IDENTIFIER, crawl_id
        )
        self.logger = logging.getLogger(self.__class__.__name__)
        self.community = community
