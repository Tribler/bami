...
from python_project.attestation.noodle.block import (
    ANY_COUNTERPARTY_PK,
    EMPTY_PK,
    GENESIS_SEQ,
    NoodleBlock,
...
from python_project.attestation.noodle.caches import (
    AuditProofRequestCache,
    ChainCrawlCache,
    CrawlRequestCache,
    HalfBlockSignCache,
    IntroCrawlTimeout,
    NoodleCrawlRequestCache,
    PingRequestCache,
    AuditRequestCache,
)
from python_project.attestation.noodle.database import NoodleDB
from python_project.attestation.noodle.exceptions import (
    InsufficientBalanceException,
    NoPathFoundException,
)
from python_project.attestation.noodle.listener import BlockListener
...
from python_project.attestation.noodle.payload import (
    ...
    HalfBlockPayload,
    AuditProofRequestPayload,
    AuditProofResponsePayload,
    AuditProofPayload,
    AuditRequestPayload,
    CrawlResponsePayload,
    CrawlRequestPayload,
    PeerCrawlRequestPayload,
    PeerCrawlResponsePayload,
    MintRequestPayload,
)
from python_project.attestation.noodle.settings import SecurityMode, NoodleSettings
...
from python_project.peerdiscovery.discovery import RandomWalk
...
from python_project.util import fail, maybe_coroutine, succeed
...
class NoodleCommunity(Community):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.debug(
            "The Noodle community started with Public Key: %s",
        ...
        if (
            hexlify(self.my_peer.public_key.key_to_bin()) in self.settings.minters
            or not self.settings.minters
        ):
    ...
    def ask_minters_for_funds(self, value=10000):
        ...
        for minter in known_minters:
            ...
            self._logger.info(
                "Sending mint request to peer %s:%d", *minter_peer.address
            )
    ...
    async def ping(self, peer):
        ...
        cache = self.request_cache.add(PingRequestCache(self, "ping", peer))
    ...
    def trustchain_sync(self, community_id):
        ...
        if community_id in self.persistence.peer_map:
            blk = self.persistence.get_last_pairwise_block(
                self.persistence.peer_map[community_id],
                self.my_peer.public_key.key_to_bin(),
            )
            if blk:
            ...
            while not p and len(self.known_graph[source]) > 0:
                paths = list(
                    nx.all_shortest_paths(
                        self.known_graph, source=source, target=target
                    )
                )
                random_path = random.choice(paths)
                if len(random_path) < 2:
                    self.logger.error(
                        "Path to key %s is less than 2 %s",
                        peer_pub_key,
                        str(random_path),
                    )
                else:
                    ...
                    if not p:
                        # p is not connected !
                        self.logger.error(
                            "Got a path, but not connected! %s. Removing the edge ",
                            random_path[1],
                        )
    ...
    async def informed_send_block(self, block1, block2=None, ttl=None, fanout=None):
        ...
        if not know_neigh:
            ...
            for neigh in know_neigh:
                paths = self.network.known_network.get_path_to_peer(
                    self.my_peer.public_key.key_to_bin(), neigh, cutoff=ttl + 1
                )
                for p in paths:
                    next_peers.add(p[1])
            ...
            if len(next_peers) < res_fanout:
                ...
                for peer in random.sample(
                    self.get_peers(), min(len(self.get_peers()), res_fanout)
                ):
            ...
            if block2:
                ...
                payload = HalfBlockBroadcastPayload.from_half_block(
                    block, ttl
                ).to_pack_list()
    ...
    def send_block_pair(self, block1, block2, address=None, address_set=None, ttl=1):
        ...
        if address:
            ...
            payload = HalfBlockPairPayload.from_half_blocks(
                block1, block2
            ).to_pack_list()
            ...
            if address_set:
                f = min(len(address_set), self.settings.broadcast_fanout)
                self.logger.debug(
                    "Broadcasting block pair in a back-channel  to %s peers", f
                )
                peers = (p.address for p in random.sample(address_set, f))
            else:
                f = min(len(self.get_peers()), self.settings.broadcast_fanout)
                self.logger.debug(
                    "Broadcasting block pair in a main-channel  to %s peers", f
                )
                peers = (p.address for p in random.sample(self.get_peers(), f))
    ...
    def self_sign_block(self, block_type=b"unknown", transaction=None):
        return self.sign_block(
            self.my_peer, block_type=block_type, transaction=transaction
        )
    ...
    def sign_block(
        ...
        assert (
            ...
            or peer is None
            and linked is None
            and public_key == ANY_COUNTERPARTY_PK
        ), (
        ...
        assert (
            ...
            "Either provide a linked block or a transaction, not both %s, %s"
            % (peer, self.my_peer)
        )
        assert (
            additional_info is None or linked is not None and transaction is None
        ), "Either no additional info is provided or one provides it for a linked block"
        assert (
            linked is None
            or linked.link_public_key == self.my_peer.public_key.key_to_bin()
            or linked.link_public_key == ANY_COUNTERPARTY_PK
        ), "Cannot counter sign block not addressed to self"
        assert (
            linked is None or linked.link_sequence_number == UNKNOWN_SEQ
        ...
        ), "Additional info should be a dictionary"
        ...
        if not self.persistence.contains(block):
            ...
            existing_block = self.persistence.get(
                block.public_key, block.sequence_number
            )
            self.hiding_blocks[block.sequence_number] = (existing_block, block)
            self.persistence.commit_block(
                block
            )  # Make sure the double spend is written away
        ...
        if peer == self.my_peer:
            ...
            return (
                succeed((block, None))
                if public_key == ANY_COUNTERPARTY_PK
                else succeed((block, linked))
            )
        ...
        if not linked:
            ...
            if not self.request_cache.has("sign", block_id_int):
    ...
    async def evaluate_incoming_block_queue(self):
        while True:
            ...
            self.logger.info(
                "Size of incoming block queue: %d", self.incoming_block_queue.qsize()
            )
    ...
    def check_local_state_wrt_block(self, block):
        ...
        if self.persistence.get_peer_proofs(peer_id, seq_num) and (
            balance < total_value or known_seq_num < seq_num
        ):
    ...
    def validate_persist_block(self, block, peer=None):
        ...
        if (
            not self.settings.ignore_validation
            and validation[0] == ValidationResult.invalid
        ):
            pass
        else:
            self.notify_listeners(block)
    ...
    @synchronized
    async def process_half_block(
        self, blk, peer, validate=True, status=None, audit_proofs=None
    ):
        """
        Process a received half block.
        """
        if blk.type == b"alert" and blk.transaction[
            "affected_peer"
        ] == self.persistence.key_to_id(self.my_peer.public_key.key_to_bin()):
            ...
            if blk.transaction["malicious_peer"] not in self.rejected_peers:
                ...
                ensure_future(
                    self.self_sign_block(
                        b"reject_interactions",
                        transaction={"peer": blk.transaction["malicious_peer"]},
                    )
                )
            return
        ...
        if status and audit_proofs:
            ...
            if not self.validate_audit_proofs(status, audit_proofs, blk):
                raise RuntimeError(
                    "Block proofs are not valid, refusing to sign: %s", blk
                )
        ...
        if self.request_cache.has("sign", link_block_id_int):
            ...
            if "condition" in blk.transaction and cache.from_peer:
                ...
                if "proof" in blk.transaction:
                    orig_blk = self.persistence.get(
                        cache.from_peer.public_key.key_to_bin(), cache.seq_num
                    )
                    ...
                    self.logger.error(
                        "Got conditional block without a proof %s ", cache.from_peer
                    )
                    raise RuntimeError(
                        "Block could not be validated: %s, %s"
                        % (validation[0], validation[1])
                    )

        linked = self.persistence.get_linked(blk)
        if (
            blk.link_sequence_number == UNKNOWN_SEQ
            and blk.link_public_key == self.my_peer.public_key.key_to_bin()
            and linked
        ):
        ...
        if blk.type == b"spend":
            if peer_id in self.rejected_peers:
                self.logger.info(
                    "Not signing block since we blocked interactions with peer %s!",
                    peer_id,
                )
                return
            ...
            if self.persistence.get_balance(peer_id) < 0 or (
                not status
                and not audit_proofs
                and not self.persistence.get_peer_proofs(peer_id, blk.sequence_number)
                ...
                if not status and not audit_proofs:
                    try:
                        ...
                        self.logger.info(
                            "Didn't receive the audit proofs - ignoring block!"
                        )
                        return
                    if status and proofs:
                        return await self.process_half_block(
                            blk,
                            peer,
                            validate=False,
                            status=status,
                    ...
                    self.logger.info(
                        "Not signing block %s due to negative balance, despite audit proofs",
                        blk,
                    )
                    return
            if "condition" in blk.transaction:
                ...
                if self.my_peer.public_key.key_to_bin() != pub_key:
                    ...
                    if not val:
                        ...
                        return addCallback(
                            self.self_sign_block(block_type=b"claim", transaction=mint),
                    ...
                    return self.sign_block(
                        next_peer,
                        next_peer.public_key.key_to_bin(),
                        transaction=new_tx,
                        block_type=blk.type,
                        from_peer=peer,
                    ...
                    sign = blk.crypto.create_signature(
                        self.my_peer.key, blk.transaction["nonce"].encode()
                    )
                    new_tx = blk.transaction
                    new_tx["proof"] = hexlify(sign).decode()
                    return self.sign_block(
                        peer, linked=blk, block_type=b"claim", additional_info=new_tx
                    )
    ...
    def validate_spend(self, spend_block, peer):
        ...
        cache = self.request_cache.get("proof-request", crawl_id)
        if not cache:
            # Need to get more information from the peer to verify the claim
            self.logger.info(
                "Requesting the status and audit proofs %s:%d from peer %s:%d",
                crawl_id,
                spend_block.sequence_number,
                peer.address[0],
                peer.address[1],
            )
            except_pack = json.dumps(list())
            if self.settings.security_mode == SecurityMode.VANILLA:
                future = self.send_peer_crawl_request(
                    crawl_id, peer, spend_block.sequence_number, except_pack
                )
            else:
                future = self.send_audit_proofs_request(
                    peer, spend_block.sequence_number, crawl_id
                )
        else:
            future = Future()
    ...
    def trigger_security_alert(self, malicious_peer_id, affected_peers, errors):
        ...
        self.logger.info(
            "Triggering security alert for malicious peer %s and affected peers %s",
            malicious_peer_id,
            affected_peers,
        )
        ...
        def send_block(future):
            ...
            my_peer_id = self.persistence.key_to_id(
                self.my_peer.public_key.key_to_bin()
            )
        ...
        for affected_peer_id in affected_peers:
            if (malicious_peer_id, affected_peer_id) not in self.made_alert:
                ...
                tx = {
                    "errors": errors,
                    "malicious_peer": malicious_peer_id,
                    "affected_peer": affected_peer_id,
                }
                self.self_sign_block(
                    block_type=b"alert", transaction=tx
                ).add_done_callback(send_block)
    ...
    def validate_audit_proofs(self, raw_status, raw_audit_proofs, block):
        ...
        self.persistence.add_peer_proofs(
            peer_id, status["seq_num"], status, raw_audit_proofs
        )
        return res
    ...
    def finalize_audits(self, audit_seq, status, audits):
        if not audits:
            self.logger.info(
                "We did not receive any audit proof from others - not finalizing this audit!"
            )
            return

        self.logger.info(
            "Audit with sequence number %d finalized (audits: %d)",
            audit_seq,
            len(audits),
        )
        ...
        for p, audit_id, proofs, status in responses_to_send:
            ...
            if audit_seq in self.hiding_blocks:
                ...
                if (
                    probable_peer.public_key.key_to_bin()
                    == self.hiding_blocks[audit_seq][1].link_public_key
                ):
                    self.logger.info(
                        "NOT sending audit proof with seq %d back to peer %s due to double spend!",
                        audit_seq,
                        probable_peer,
                    )
                    continue
    ...
    async def trustchain_active_sync(self, community_mid):
        # choose the peers
        self.logger.info(
            "Active Sync asking in the community %s", hexlify(community_mid).decode()
        )
        ...
        selected_peers = self.choose_community_peers(
            peer_list, seed, min(self.settings.com_size, len(peer_list))
        )
        if not selected_peers:
            self.logger.info(
                "There are no peers in the community to ask for an audit, skipping audit for now."
            )
            return
        ...
        if self.persistence.get_peer_proofs(my_id, seq_num):
            self.logger.info(
                "Skipping audit since we already have proofs for our last block."
            )
            return
        ...
        if self.hiding_blocks:
            ...
            if status_seq_num in self.hiding_blocks:
                ...
                shadow_peer_status = self.form_peer_status_response(
                    peer_key, self.hiding_blocks[status_seq_num]
                )
                shadow_peer_status_json = json.dumps(shadow_peer_status)
                shadow_crawl_id = self.persistence.id_to_int(
                    self.persistence.key_to_id(peer_key + b"a")
                )
                self.logger.info(
                    "Requesting a shadow audit for sequence number %d from %d peers",
                    seq_num,
                    len(selected_peers),
                )
                audit_future = Future()
                audit_future.add_done_callback(
                    lambda future, sseq_num=seq_num, status_json=shadow_peer_status_json: self.on_shadow_audit_done(
                        sseq_num, status_json, future.result()
                    )
                )
                self.request_cache.add(
                    AuditRequestCache(
                ...
                for peer in selected_peers:
                    self.send_audit_request(
                        peer.address, shadow_crawl_id, seq_num, shadow_peer_status_json
                    )
        ...
        if not self.request_cache.get("audit", crawl_id):
            audit_future = Future()
            self.request_cache.add(
                AuditRequestCache(
                    self,
            ...
            self.logger.info(
                "Requesting an audit for sequence number %d from %d peers",
                seq_num,
                len(selected_peers),
            )
            for peer in selected_peers:
                self.send_audit_request(
                    peer.address, crawl_id, seq_num, peer_status_json
                )
    ...
    def on_shadow_audit_done(self, seq_num, peer_status_json, audits):
        self.logger.info(
            "Shadow audit with sequence number %d finalized (audits: %d)",
            seq_num,
            len(audits),
        )
        if seq_num in self.shadow_proof_requests:
            ...
            proofs = json.dumps(full_audit)
            ensure_future(
                self.respond_with_audit_proof(
                    peer_address, audit_id, proofs, peer_status_json, is_shadow=True
                )
            )
    ...
    async def send_audit_proofs_request(self, peer, seq_num, audit_id):
        ...
        """
        self._logger.debug(
            "Sending audit proof request to peer %s:%d (seq num: %d, id: %s)",
            peer.address[0],
            peer.address[1],
            seq_num,
            audit_id,
    ...
    def received_audit_proofs_request(self, source_address, dist, payload, data):
        ...
        if pack:
            ...
            ensure_future(
                self.respond_with_audit_proof(
                    source_address, payload.crawl_id, proofs, status
                )
            )
        else:
            ...
            self._logger.info(
                "Adding audit proof request from %s:%d (id: %d) to cache",
                source_address[0],
                source_address[1],
                payload.crawl_id,
            )
            ...
            if payload.seq_num in self.hiding_blocks:
                ...
                if (
                    probable_peer.public_key.key_to_bin()
                    == self.hiding_blocks[payload.seq_num][1].link_public_key
                ):
            ...
            self.proof_requests[payload.seq_num].append(
                (source_address, payload.crawl_id)
            )

    async def respond_with_audit_proof(
        self, address, audit_id, proofs, status, is_shadow=False
    ):
        """
        Send audit proofs and status back to a specific peer, based on a request.
        """
        self.logger.info(
            "Responding with audit proof %s to peer %s:%d (shadow? %s)",
            audit_id,
            address[0],
            address[1],
            is_shadow,
        ...
        for item in [proofs, status]:
            global_time = self.claim_global_time()
            payload = AuditProofResponsePayload(
                audit_id, item, item == proofs
            ).to_pack_list()
            dist = GlobalTimeDistributionPayload(global_time).to_pack_list()
    ...
    def received_audit_proofs_response(self, source_address, dist, payload, data):
        self.logger.info(
            "Received audit proof response from %s:%d (id: %s, proofs? %s)",
            source_address[0],
            source_address[1],
            payload.audit_id,
            payload.is_proof,
        )
        cache = self.request_cache.get("proof-request", payload.audit_id)
        if cache:
            if payload.is_proof:
            ...
            self.logger.info(
                "Received audit proof response for non-existent cache with id %s",
                payload.audit_id,
            )
    ...
    def received_audit_proofs(self, source_address, dist, payload, data):
        cache = self.request_cache.get("audit", payload.audit_id)
        if cache:
            self._logger.info(
                "Adding audit proofs from %s:%d (id: %d) to cache",
                source_address[0],
                source_address[1],
                payload.audit_id,
            )
            ...
            self.logger.info(
                "Received audit proof for non-existent cache with id %s",
                payload.audit_id,
            )
    ...
    def send_audit_request(self, address, crawl_id, seq_num, peer_status):
        ...
        peer_id = self.persistence.key_to_id(self.my_peer.public_key.key_to_bin())
        self._logger.info(
            "Sending audit request for seq num %d to peer %s:%d (my id: %s)",
            seq_num,
            address[0],
            address[1],
            peer_id,
        ...
        payload = AuditRequestPayload(
            crawl_id, peer_status, peer_id.encode()
        ).to_pack_list()
        dist = GlobalTimeDistributionPayload(global_time).to_pack_list()
    ...
    def received_audit_request(self, peer, dist, payload):
        ...
        self.logger.info(
            "Received audit request %s from peer %s:%d --> %s",
            payload.audit_id,
            peer.address[0],
            peer.address[1],
            payload.peer_status,
    ...
    def perform_audit(self, source_address, audit_request):
        ...
        try:
            ...
            if (
                result.state == ValidationResult.invalid
                and not self.settings.is_malicious_witness
            ):
                # Alert: Peer is provably hiding a transaction
                self.logger.error("Peer is hiding transactions: %s", result.errors)
                self.trigger_security_alert(peer_id, affected_peers, result.errors)
                ...
                if self.persistence.dump_peer_status(peer_id, peer_status):
                    ...
                    self.persistence.add_peer_proofs(
                        peer_id, seq_num, peer_status, None
                    )
                    # Create an audit proof for the this sequence
                    signature = default_eccrypto.create_signature(
                        self.my_peer.key, audit_request.peer_status
                    )
                    # create an audit proof
                    audit = {}
                    my_id = hexlify(self.my_peer.public_key.key_to_bin()).decode()
                    audit[my_id] = hexlify(signature).decode()
                    self.send_audit_proofs(
                        source_address, audit_request.audit_id, json.dumps(audit)
                    )
                else:
                    # This is invalid audit request, refusing to sign
                    self.logger.error(
                        "Received invalid audit request id %s", audit_request.crawl_id
                    )
        except JSONDecodeError:
            self.logger.info(
                "Invalid JSON received in audit request from peer %s:%d!",
                source_address[0],
                source_address[1],
            )
    ...
    def send_audit_proofs(self, address, audit_id, audit_proofs):
        ...
        """
        self.logger.info(
            "Sending audit proof %s to peer %s:%d", audit_id, address[0], address[1]
        )
        global_time = self.claim_global_time()
    ...
    def received_mint_request(self, peer, dist, payload):
        self._logger.info(
            "Received mint request with value %d from peer %s", payload.mint_value, peer
        )
    ...
    def received_peer_crawl_response(self, peer, dist, payload):
        cache = self.request_cache.get("noodle-crawl", payload.crawl_id)
        if cache:
            ...
            if result.state == ValidationResult.invalid:
                ...
                self.logger.info(
                    "Dump chain for %s, balance after is %s", peer_id, after_balance
                )
                if after_balance < 0:
    ...
    def send_peer_crawl_response(self, peer, crawl_id, chain):
        ...
        """
        self._logger.info(
            "Sending peer crawl response to peer %s:%d",
            peer.address[0],
            peer.address[1],
        )
        global_time = self.claim_global_time()
    ...
    def form_peer_status_response(self, public_key, hiding_blocks=None):
        ...
        if hiding_blocks:
            ...
            existing_block_peer_id = self.persistence.key_to_id(
                existing_block.link_public_key
            )
            shadow_block_peer_id = self.persistence.key_to_id(
                shadow_block.link_public_key
            )
            status["spends"].pop(existing_block_peer_id)
            # Add the other one
            status["spends"][shadow_block_peer_id] = [
                shadow_block.transaction["value"],
                shadow_block.sequence_number,
            ]
        return status
    ...
    def received_peer_crawl_request(self, peer, dist, payload):
        ...
        self.logger.info(
            "Received peer crawl from node %s for range, sending status len %s",
    ...
    async def send_peer_crawl_request(self, crawl_id, peer, seq_num, pack_except):
        ...
        self.logger.info(
            "Requesting balance proof for peer %s at seq num %d with id %d",
            hexlify(peer.public_key.key_to_bin())[-8:],
            seq_num,
            crawl_id,
        )
    ...
    def send_crawl_request(
        self, peer, public_key, start_seq_num, end_seq_num, for_half_block=None
    ):
        """
        Send a crawl request to a specific peer.
        """
        crawl_id = (
            for_half_block.hash_number
            if for_half_block
            else RandomNumberCache.find_unclaimed_identifier(
                self.request_cache, "crawl"
            )
        )
        crawl_future = Future()
        ...
        payload = CrawlRequestPayload(
            public_key, start_seq_num, end_seq_num, crawl_id
        ).to_pack_list()
        dist = GlobalTimeDistributionPayload(global_time).to_pack_list()
    ...
    async def perform_partial_chain_crawl(self, cache, start, stop):
        ...
        await self.send_crawl_request(
            cache.peer, cache.peer.public_key.key_to_bin(), start, stop
        )
        ...
        if not latest_block:
            ...
            if latest_block.sequence_number < cache.known_chain_length:
                self.perform_partial_chain_crawl(
                    cache, latest_block.sequence_number + 1, cache.known_chain_length
                )
            else:
                self.request_cache.pop("chaincrawl", cache.number)
                cache.crawl_future.set_result(None)
    ...
    def received_crawl_request(self, peer, dist, payload):
        self.logger.info(
            "Received crawl request from node %s for range %d-%d",
            hexlify(peer.public_key.key_to_bin())[-8:],
            payload.start_seq_num,
            payload.end_seq_num,
        )
        ...
        if start_seq_num < 0:
            ...
            start_seq_num = (
                max(GENESIS_SEQ, last_block.sequence_number + start_seq_num + 1)
                if last_block
                else GENESIS_SEQ
            )
        if end_seq_num < 0:
            last_block = self.persistence.get_latest(payload.public_key)
            end_seq_num = (
                max(GENESIS_SEQ, last_block.sequence_number + end_seq_num + 1)
                if last_block
                else GENESIS_SEQ
            )
        ...
        if total_count == 0:
            global_time = self.claim_global_time()
            response_payload = EmptyCrawlResponsePayload(
                payload.crawl_id
            ).to_pack_list()
            dist = GlobalTimeDistributionPayload(global_time).to_pack_list()
    ...
    def sanitize_database(self):
        ...
        while (
            ...
            validation = self.validate_persist_block(block)
        self.logger.error(
            "Recovered database, our last block is now %d",
    ...
    def send_crawl_response(self, block, crawl_id, index, total_count, peer):
        ...
        global_time = self.claim_global_time()
        payload = CrawlResponsePayload.from_crawl(
            block, crawl_id, index, total_count
        ).to_pack_list()
        dist = GlobalTimeDistributionPayload(global_time).to_pack_list()
    ...
    async def received_crawl_response(self, source_address, dist, payload, data):
        await self.received_half_block(
            source_address, data[:-12]
        )  # We cut off a few bytes to make it a BlockPayload
    ...
    def received_empty_crawl_response(self, source_address, dist, payload, data):
        ...
        if cache:
            self.logger.info(
                "Received empty crawl response for crawl with ID %d", payload.crawl_id
            )
            cache.received_empty_response()
    ...
    def create_introduction_request(self, socket_address, extra_bytes=b""):
        ...
        return super(NoodleCommunity, self).create_introduction_request(
    ...
    def create_introduction_response(
        ...
        extra_bytes = struct.pack(">l", self.get_chain_length())
        return super(NoodleCommunity, self).create_introduction_response(
            lan_socket_address, socket_address, identifier, introduction, extra_bytes
        )

    def build_security_community(self, community_mid):
        # Start sync task after the discovery
        task = (
            self.trustchain_sync
            if self.settings.security_mode == SecurityMode.VANILLA
    ...
    def init_minter_community(self):
        if (
            self.my_peer.mid not in self.pex
            and hexlify(self.my_peer.public_key.key_to_bin())
            not in self.settings.crawlers
        ):
    ...
    def form_subtrust_community(self, peer):
        ...
        if hexlify(self.my_peer.public_key.key_to_bin()) in self.settings.crawlers:
            ...
            peer.public_key.key_to_bin() in known_minters or not self.settings.minters
        ) and peer.mid not in self.pex:
            self.logger.info("Creating SubTrustCommunity around peer %s", peer)
            community = SubTrustCommunity(
                self.my_peer,
                self.ipv8.endpoint,
                Network(),
                master_peer=peer,
            ...
            if self.bootstrap_master:
                ...
                self.ipv8.strategies.append(
                    (RandomWalk(community), self.settings.max_peers_subtrust)
                )
