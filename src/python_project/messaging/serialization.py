...
class Bits(object):
    def pack(
        self, bit_7=0, bit_6=0, bit_5=0, bit_4=0, bit_3=0, bit_2=0, bit_1=0, bit_0=0
    ):
        """
...
class Serializer(object):
    ...
    def pack_multiple(self, pack_list):
        ...
        for packable in pack_list:
            try:
                ...
                raise PackError(
                    "Could not pack item %d: %s\n%s: %s"
                    % (index, repr(packable), type(e).__name__, str(e))
                ) from e
            index += 1
        return out, size
    ...
    def ez_pack_serializables(self, serializables):
        ...
        :rtype: bytes or str
        """
        out, _ = self.pack_multiple(
            list(
                itertools.chain.from_iterable(
                    serializable.to_pack_list() for serializable in serializables
                )
            )
        )
        return out
    ...
    def unpack_multiple(self, unpack_list, data, optional_list=[], offset=0):
        ...
        for format in unpack_list + optional_list:
            ...
            try:
                ...
                raise PackError(
                    "Could not pack item %d: %s\n%s: %s"
                    % (index, format, type(e).__name__, str(e))
                ) from e
    ...
    def unpack_to_serializables(self, serializables, data):
        ...
        for serializable in serializables:
            try:
                unpack_list, offset = self.unpack_multiple(
                    serializable.format_list,
                    data,
                    serializable.optional_format_list,
                    offset,
                )
            except Exception as e:
                raise PackError(
                    "Failed to unserialize %s\n%s: %s"
                    % (serializable.__name__, type(e).__name__, str(e))
                ) from e
    ...
    def ez_unpack_serializables(self, serializables, data):
        ...
        if unknown_data:
            raise PackError(
                "Incoming packet %s (%s) has extra data: (%s)"
                % (
                    str(
                        [
                            serializable_class.__name__
                            for serializable_class in serializables
                        ]
                    ),
                    hexlify(data),
