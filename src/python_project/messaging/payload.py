...
from python_project.messaging.serialization import Serializable
from typing import List, Tuple, Union


def encode_connection_type(connection_type: str) -> Tuple[int, int]:
    if connection_type == u"public":
        return 1, 0
    if connection_type == u"symmetric-NAT":
        return 1, 1
    return 0, 0
...
class Payload(Serializable):
    def __str__(self):
        ...
        for attribute in dir(self):
            if not (
                attribute.startswith("_") or callable(getattr(self, attribute))
...
class IntroductionRequestPayload(Payload):
    ...
    def __init__(
        ...
        source_lan_address: Tuple[str, int],
        source_wan_address: Tuple[str, int],
        advice: bool,
        connection_type: str,
        identifier: int,
        extra_bytes: bytes,
    ...
    def to_pack_list(
        ...
        Union[
            Tuple[str, bytes, int],
            Tuple[str, int, int, int, int, int, int, int, bool],
            Tuple[str, int],
            Tuple[str, bytes],
        ]
    ]:
        ...
        data = [
            (
                "4SH",
                inet_aton(self.destination_address[0]),
            ...
            (
                "bits",
                encoded_connection_type[0],
                encoded_connection_type[1],
                0,
                0,
                0,
                0,
    ...
    def from_unpack_list(
        cls,
        destination_address: List[Union[bytes, int]],
        source_lan_address: List[Union[bytes, int]],
        source_wan_address: List[Union[bytes, int]],
...
class IntroductionResponsePayload(Payload):
    ...
    def __init__(
        ...
        source_lan_address: Tuple[str, int],
        source_wan_address: Tuple[str, int],
        lan_introduction_address: Tuple[str, int],
        wan_introduction_address: Tuple[str, int],
        connection_type: str,
        tunnel: bool,
        identifier: int,
        extra_bytes: bytes,
    ...
    def to_pack_list(
        ...
        data = [
            ...
            (
                "4SH",
                inet_aton(self.lan_introduction_address[0]),
                self.lan_introduction_address[1],
            ),
            (
                "4SH",
                inet_aton(self.wan_introduction_address[0]),
                self.wan_introduction_address[1],
            ),
            (
                "bits",
                encoded_connection_type[0],
    ...
    def from_unpack_list(
        ...
        source_lan_address: List[Union[bytes, int]],
        source_wan_address: List[Union[bytes, int]],
        introduction_lan_address: List[Union[bytes, int]],
        introduction_wan_address: List[Union[bytes, int]],
        connection_type_0: int,
        connection_type_1: int,
        dflag0: int,
        dflag1: int,
...
class PunctureRequestPayload(Payload):
    ...
    def __init__(
        self,
        lan_walker_address: Tuple[str, int],
        wan_walker_address: Tuple[str, int],
        identifier: int,
    ) -> None:
        """
    ...
    def to_pack_list(self) -> List[Union[Tuple[str, bytes, int], Tuple[str, int]]]:
        data = [
            ("4SH", inet_aton(self.lan_walker_address[0]), self.lan_walker_address[1]),
    ...
    def from_unpack_list(
        cls,
        lan_walker_address: List[Union[bytes, int]],
        wan_walker_address: List[Union[bytes, int]],
        identifier: int,
    ) -> PunctureRequestPayload:
        args = [
            (inet_ntoa(lan_walker_address[0]), lan_walker_address[1]),
