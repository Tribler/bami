...
class VariablePayload(Payload):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not issubclass(
            super(VariablePayload, self).__class__, VariablePayload
        ) and inspect.ismethod(super(VariablePayload, self).__init__):
            super_argspec = inspect.getargspec(
                super(VariablePayload, self).__init__
            ).args[1:]
        ...
        if index != len(self.format_list):
            raise KeyError(
                "%s missing %d arguments!"
                % (self.__class__.__name__, len(args) - index)
            )
        ...
        if kwargs:
            raise KeyError(
                "%s has leftover keyword arguments: %s!"
    ...
    def to_pack_list(self):
        ...
        for _ in range(len(self.format_list)):
            out.append(
                (
                    self._to_packlist_fmt(self.format_list[index]),
        ...
        while index < len(self.names) and hasattr(self, self.names[index]):
            out.append(
                (
                    self._to_packlist_fmt(
                        self.optional_format_list[index - len(self.format_list)]
                    ),
                    self._fix_pack(self.names[index]),
                )
            )
...
def __init__(self, %s):
    ...
    """ % (
        ", ".join(
            [name for name in names[:fmt_list_len]]
            + ["%s=None" % name for name in names[fmt_list_len:]]
        ),
        "\n    ".join(
            ["self.%s = %s" % (name, name) for name in names[:fmt_list_len]]
            + [
                """if %s is not None:
        setattr(self, "%s", %s)
        ...
        opt_args += 1"""
                % (
                    ...
                    if isinstance(src_cls.optional_format_list[i], str)
                    else '"payload"',
...
def from_unpack_list(cls, %s):
    ...
    """ % (
        ...
        ", ".join(
            [
                "None if %s is None else cls.fix_unpack_%s(%s)" % (name, name, name)
...
def to_pack_list(self):
    return [%s] + self._opt_list
        """ % ", ".join(
        (
            '("%s", self.fix_pack_%s(self.%s))'
            % (fmt if isinstance(fmt, str) else "payload", names[i], names[i])
        )
        if hasattr(src_cls, "fix_pack_" + names[i])
        else (
...
def vp_compile(vp_definition):
    ...
    exec(
        _compile_init(
            vp_definition, len(vp_definition.format_list), vp_definition.names
        ),
        globals(),
        locals(),
    )
    exec(
        _compile_from_unpack_list(
            vp_definition, len(vp_definition.format_list), vp_definition.names
        ),
        globals(),
        locals(),
    ...
    exec(
        _compile_to_pack_list(
            vp_definition, vp_definition.format_list, vp_definition.names
        ),
        globals(),
        locals(),
    )
    ...
    setattr(
        vp_definition,
        "from_unpack_list",
        types.MethodType(locals()["from_unpack_list"], vp_definition),
    )
