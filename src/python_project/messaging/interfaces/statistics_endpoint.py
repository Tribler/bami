...
class StatisticsEndpoint(EndpointListener):
    ...
    def add_sent_stat(self, prefix, identifier, num_bytes, timestamp=None):
        ...
        self.statistics[prefix][identifier].add_sent_stat(
            timestamp if timestamp else time.time(), num_bytes
        )
    ...
    def add_received_stat(self, prefix, identifier, num_bytes, timestamp=None):
        ...
        if identifier not in self.statistics[prefix]:
            self.statistics[prefix][identifier] = NetworkStat(identifier)
        self.statistics[prefix][identifier].add_received_stat(
            timestamp if timestamp else time.time(), num_bytes
        )
        ...
        if prefix in self.statistics:
            ...
            for _, network_stat in self.statistics[prefix].items():
                ...
                first_ts = min_positive(
                    first_ts,
                    min_positive(
                        network_stat.first_measured_up, network_stat.first_measured_down
                    ),
                )
                last_ts = max(
                    last_ts,
                    max(network_stat.last_measured_up, network_stat.last_measured_down),
                )
    ...
    def is_excluded(
        self, identifier, include_introduction, include_puncture, include_deprecated
    ):
        return (
            identifier in StatisticsEndpoint.IDS_DEPRECATED
            and not include_deprecated
            or identifier in StatisticsEndpoint.IDS_INTRODUCTION
            and not include_introduction
            or identifier in StatisticsEndpoint.IDS_PUNCTURE
            and not include_puncture
        )
