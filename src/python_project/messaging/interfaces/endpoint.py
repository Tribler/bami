...
class Endpoint(metaclass=abc.ABCMeta):
    ...
    def add_prefix_listener(self, listener, prefix):
        ...
        if not len(prefix) == self.prefixlen:
            raise RuntimeError(
                "Tried to register a prefix of length %d, required to be of length %d!"
                % (len(prefix), self.prefixlen)
            )
        with self.listener_update_lock:
            self._prefix_map[prefix] = (
                self._prefix_map.get(prefix, []) + [listener] + self._listeners
            )
    ...
    def _deliver_later(self, listener, packet):
        ...
        if self.is_open() and (
            packet[1][: self.prefixlen] in self._prefix_map
            or listener in self._listeners
        ):
...
class EndpointListener(metaclass=abc.ABCMeta):
    ...
    def _get_interface_addresses():
        ...
        class Interface(object):
            ...
            def __contains__(self, address):
                ...
                return (l_address & self._l_netmask) == (
                    self._l_address & self._l_netmask
                )

            def __str__(self):
                return '<{obj.__class__.__name__} "{obj.name}" addr:{obj.address} mask:{obj.netmask}>'.format(
                    obj=self
                )
    ...
    def _address_is_lan_without_netifaces(self, address):
        ...
        if address == self.get_lan_address_without_netifaces():
            ...
            return any(
                self._address_in_subnet(address, subnet) for subnet in lan_subnets
            )
    ...
    def get_lan_address_without_netifaces(self):
        ...
        try:
            ...
            s.connect(
                ("192.0.2.0", 80)
            )  # TEST-NET-1, guaranteed to not be connected => no callbacks
        ...
        if self._netifaces_failed:
            ...
            return (
                (
                    interface.address
                    if interface
                    else self.get_lan_address_without_netifaces()
                ),
    ...
    def _guess_lan_address(self, interfaces, default=None):
        ...
        for interface in interfaces:
            if (
                ...
                and interface.address not in blacklist
            ):
                return interface
...
class IllegalEndpointListenerError(RuntimeError):
    ...
    def __init__(self, other):
        message = "%s is not an instance of %s" % (
            type(other),
            str(EndpointListener.__name__),
        )
...
class EndpointClosedException(Exception):
    ...
    def __init__(self, endpoint):
        super(EndpointClosedException, self).__init__(
            "%s is unexpectedly closed" % type(endpoint)
        )
...
class DataTooBigException(Exception):
    ...
    def __init__(self, size, max_size):
        super(DataTooBigException, self).__init__(
            "Tried to send packet of size %s > MAX_SIZE(%d)" % (size, max_size)
        )
...
class IllegalDestination(Exception):
    ...
    def __init__(self):
        super(IllegalDestination, self).__init__(
            "Attempted to send a message to 0.0.0.0:0."
        )
