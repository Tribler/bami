...
class TunnelEndpoint(object):
    ...
    def send(self, address, packet):
        ...
        if self.tunnel_community:
            circuits = self.tunnel_community.find_circuits(
                ctype=CIRCUIT_TYPE_IPV8, hops=self.hops, state=None
            )
            circuit = circuits[0] if circuits else None
            if not circuit or circuit.state != CIRCUIT_STATE_READY:
                # Recreate tunnel when needed
                if not circuit:
                    self.tunnel_community.create_circuit(
                        self.hops, ctype=CIRCUIT_TYPE_IPV8
                    )
            ...
            self.tunnel_community.send_data(
                (circuit_address,), circuit_id, address, ("0.0.0.0", 0), packet
            )
