...
class DataChecker(object):
    ...
    def could_be_udp_tracker(data):
        ...
        if (
            len(data) >= 8
            and (0 <= unpack_from("!I", data, 0)[0] <= 3)
            or len(data) >= 12
            and (0 <= unpack_from("!I", data, 8)[0] <= 3)
        ):
            return True
        return False
    ...
    def could_be_ipv8(data):
        return (
            len(data) >= 23 and data[0:1] == b"\x00" and data[1:2] in [b"\x01", b"\x02"]
        )

    @staticmethod
    def is_allowed(data):
        return (
            DataChecker.could_be_utp(data)
    ...
    def enable(self):
        if not self.enabled:
            ...
            async def create_transport():
                self.transport, _ = await get_event_loop().create_datagram_endpoint(
            ...
            if DataChecker.is_allowed(data):
                ...
                def on_ip_address(future):
                    try:
                        ...
                        self.logger.error(
                            "Can't resolve ip address for %s. Failure: %s",
                            destination[0],
                            e,
                        )
                        return
                    ...
                    self.logger.debug(
                        "Resolved hostname %s to ip_address %s",
                        destination[0],
                        ip_address,
                    )
                    try:
                        ...
                        self.logger.error(
                            "Failed to write to transport. Destination: %r error: %r",
                            destination,
                            e,
                        )
                ...
                try:
                    ...
                    self.register_anonymous_task(
                        "resolving_%r" % destination[0],
                        task,
                        ignore=(OSError, ValueError),
                    ).add_done_callback(on_ip_address)
        ...
        if self.check_num_packets(source, True):
            if DataChecker.is_allowed(data):
                try:
                    ...
                    self.logger.error(
                        "Exception occurred while handling incoming exit node data!\n"
                        + "".join(format_exception(*sys.exc_info()))
                    )
            else:
                self.logger.warning(
                    "Dropping forbidden packets to exit socket with circuit_id %d",
                    self.circuit_id,
                )

    def tunnel_data(self, source, data):
        self.logger.debug(
            "Tunnel data to origin %s for circuit %s", ("0.0.0.0", 0), self.circuit_id
        )
        self.overlay.send_data(
            [self.peer], self.circuit_id, ("0.0.0.0", 0), source, data
        )
...
class IntroductionPoint(object):
    def __init__(
        self, peer, seeder_pk, source=PEER_SOURCE_UNKNOWN, last_seen=int(time.time())
    ):
        self.peer = peer
...
class Swarm(object):
    def __init__(
        ...
        seeder_sk=None,
        max_ip_age=180,
        min_dht_lookup_interval=300,
        max_dht_lookup_interval=120,
    ):
        self.info_hash = info_hash
        self.hops = hops
    ...
    def _active_circuits(self):
        return [
            c
            for c, _ in self.connections.values()
            if c.state == CIRCUIT_STATE_READY and c.e2e
        ]
    ...
    def remove_old_intro_points(self):
        ...
        used_intro_points = [
            i
            for c, i in self.connections.values()
            if c.state == CIRCUIT_STATE_READY and c.e2e
        ]
        self.intro_points = [
            i
            for i in self.intro_points
    ...
    async def lookup(self, target=None):
        ...
        if target:
            self.logger.info(
                "Performing manual PEX lookup for swarm %s (target %s)",
                hexlify(self.info_hash),
                target.peer,
            )
        ...
        if (now - self.last_dht_response) > self.min_dht_lookup_interval or (
            not self.intro_points
            and (now - self.last_dht_response) > self.max_dht_lookup_interval
        ):
            self.logger.info(
                "Performing DHT lookup for swarm %s", hexlify(self.info_hash)
            )
            ...
            self.logger.info(
                "Performing PEX lookup for swarm %s (target %s)",
                hexlify(self.info_hash),
                target.peer,
            )
            return on_success(await self.lookup_func(self.info_hash, target, self.hops))
    ...
    def get_total_up(self):
        return (
            sum([c.bytes_up for c in self._active_circuits]) + self.transfer_history[0]
        )

    def get_total_down(self):
        return (
            sum([c.bytes_down for c in self._active_circuits])
            + self.transfer_history[1]
        )
