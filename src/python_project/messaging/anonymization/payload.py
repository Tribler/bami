...
def encode_address(host, port):
    ...
    if is_ip:
        ...
        return (
            pack("!BH", ADDRESS_TYPE_DOMAIN_NAME, len(host))
            + cast_to_bin(host)
            + pack("!H", port)
        )
...
class DataPayload(object):
    ...
    def to_bin(self):
        ...
        return b"".join(
            [pack("!H", len(dest)), dest, pack("!H", len(org)), org, self.data]
        )
...
class CellPayload(object):
    ...
    def encrypt(self, crypto, circuit=None, relay_session_keys=None):
        ...
        if circuit:
            if circuit.hs_session_keys and circuit.ctype in [
                CIRCUIT_TYPE_RP_SEEDER,
                CIRCUIT_TYPE_RP_DOWNLOADER,
            ]:
                direction = int(circuit.ctype == CIRCUIT_TYPE_RP_SEEDER)
                self.message = crypto.encrypt_str(
                    self.message,
            ...
            for hop in reversed(circuit.hops):
                self.message = crypto.encrypt_str(
                    self.message, *crypto.get_session_keys(hop.session_keys, EXIT_NODE)
                )

        elif relay_session_keys:
            self.message = crypto.encrypt_str(
                self.message, *crypto.get_session_keys(relay_session_keys, ORIGINATOR)
            )

        else:
            raise CryptoException(
                "Error encrypting message for unknown circuit %d" % self.circuit_id
            )
    ...
    def decrypt(self, crypto, circuit=None, relay_session_keys=None):
        ...
        if circuit:
            if not circuit.hops:
                raise CryptoException(
                    "Error decrypting message for 0-hop circuit %d" % self.circuit_id
                )

            # Remove all the encryption layers
            for layer, hop in enumerate(circuit.hops):
                try:
                    self.message = crypto.decrypt_str(
                        self.message,
                    ...
                    raise CryptoException(
                        "Got exception %r when trying to remove encryption layer %s "
            ...
            try:
                ...
                raise CryptoException(
                    "Got exception %r when trying to decrypt relay message: "
                    "cell received for circuit_id: %s" % (e, self.circuit_id)
                )

        else:
            raise CryptoException(
                "Error decrypting message for unknown circuit %d" % self.circuit_id
            )

    def unwrap(self, prefix):
        return b"".join(
            [prefix, self.message[0:1], pack("!I", self.circuit_id), self.message[1:]]
        )
...
class RendezvousEstablishedPayload(Payload):
    ...
    def from_unpack_list(cls, circuit_id, identifier, address):
        ...
        return RendezvousEstablishedPayload(
            circuit_id, identifier, rendezvous_point_addr
        )
