...
class CreateRequestCache(NumberCache):
    ...
    def __init__(self, community, to_circuit_id, from_circuit_id, peer, to_peer):
        super(CreateRequestCache, self).__init__(
            community.request_cache, u"create", to_circuit_id
        )
...
class CreatedRequestCache(NumberCache):
    ...
    def __init__(self, community, circuit_id, candidate, candidates, timeout):
        super(CreatedRequestCache, self).__init__(
            community.request_cache, u"created", circuit_id
        )
...
class RetryRequestCache(NumberCache):
    ...
    def __init__(self, community, circuit, candidates, max_tries, retry_func, timeout):
        super(RetryRequestCache, self).__init__(
            community.request_cache, u"retry", circuit.circuit_id
        )
        self.community = community
    ...
    def on_timeout(self):
        ...
        self.community.request_cache.register_anonymous_task(
            "retry-later", retry_later, delay=0
        )
...
class IPRequestCache(RandomNumberCache):
    def __init__(self, community, circuit):
        super(IPRequestCache, self).__init__(
            community.request_cache, u"establish-intro"
        )
        self.logger = logging.getLogger(__name__)
        self.circuit = circuit
        self.community = community
    ...
    def on_timeout(self):
        self.logger.info(
            "IPRequestCache: no response on establish-intro (circuit %d)",
            self.circuit.circuit_id,
        )
        self.community.remove_circuit(
            self.circuit.circuit_id, "establish-intro timeout"
        )


class RPRequestCache(RandomNumberCache):
    def __init__(self, community, rp):
        super(RPRequestCache, self).__init__(
            community.request_cache, u"establish-rendezvous"
        )
        self.logger = logging.getLogger(__name__)
        self.community = community
        self.rp = rp
    ...
    def on_timeout(self):
        self.logger.info(
            "RPRequestCache: no response on establish-rendezvous (circuit %d)",
            self.rp.circuit.circuit_id,
        )
        self.rp.ready.set_result(None)
        self.community.remove_circuit(
            self.rp.circuit.circuit_id, "establish-rendezvous timeout"
        )


class PeersRequestCache(RandomNumberCache):
    def __init__(self, community, circuit, info_hash):
        super(PeersRequestCache, self).__init__(
            community.request_cache, u"peers-request"
        )
        self.circuit = circuit
