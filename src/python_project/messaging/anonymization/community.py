...
class TunnelCommunity(Community):
    ...
    master_peer = Peer(
        unhexlify(
            "4c69624e61434c504b3adbd575f1902f1d39debeb3d7b576dff1652ab6cede21c14545d134219b57b"
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.decode_map.update({chr(1): self.on_cell, chr(10): self.on_destroy})
        ...
        self.num_hops_by_downloads = defaultdict(
            int
        )  # Keeps track of the number of hops required by downloads
        self.candidates = (
            {}
        )  # Keeps track of the candidates that want to be a relay/exit node

        self.crypto = self.settings.crypto

        self.logger.info(
            "Setting exitnode = %s", self.settings.peer_flags & PEER_FLAG_EXIT_ANY
        )
    ...
    async def unload(self):
        ...
        for circuit_id in list(self.relay_from_to.keys()):
            self.remove_relay(
                circuit_id, "unload", remove_now=True, destroy=True, both_sides=False
            )
    ...
    def do_circuits(self):
        for circuit_length, num_circuits in self.circuits_needed.items():
            num_to_build = max(
                0,
                num_circuits - len(self.find_circuits(state=None, hops=circuit_length)),
            )
            self.logger.info(
                "Want %d data circuits of length %d", num_to_build, circuit_length
            )
            for _ in range(num_to_build):
                if not self.create_circuit(circuit_length):
                    self.logger.info(
                        "circuit creation of %d circuits failed, no need to continue",
                        num_to_build,
                    )
                    break
        self.do_remove()
    ...
    def tunnels_ready(self, hops):
        if hops > 0 and self.circuits_needed.get(hops, 0):
            return len(self.find_circuits(hops=hops)) / float(
                self.circuits_needed[hops]
            )
        return 1.0
    ...
    def do_remove(self):
        ...
        for circuit_id, circuit in list(self.circuits.items()):
            if (
                ...
                and circuit.last_activity
                < time.time() - self.settings.max_time_inactive
            ):
        ...
        for circuit_id, relay in list(self.relay_from_to.items()):
            if relay.last_activity < time.time() - self.settings.max_time_inactive:
                ...
                self.remove_relay(
                    circuit_id, "traffic limit exceeded", both_sides=False
                )
        ...
        for circuit_id, exit_socket in list(self.exit_sockets.items()):
            if (
                exit_socket.last_activity
                < time.time() - self.settings.max_time_inactive
            ):
                self.remove_exit_socket(circuit_id, "no activity")
            elif exit_socket.creation_time < time.time() - self.get_max_time(
                circuit_id
            ):
                self.remove_exit_socket(circuit_id, "too old")
            elif (
                exit_socket.bytes_up + exit_socket.bytes_down
                > self.settings.max_traffic
            ):
    ...
    def get_candidates(self, flag):
        return [
            peer
            for peer, flags in self.candidates.items()
    ...
    def find_circuits(
        self, ctype=CIRCUIT_TYPE_DATA, state=CIRCUIT_STATE_READY, hops=None
    ):
        return [
            c
    ...
    def create_circuit(
        self, goal_hops, ctype=CIRCUIT_TYPE_DATA, required_exit=None, info_hash=None
    ):
        self.logger.info(
            "Creating a new circuit of length %d (type: %s)", goal_hops, ctype
        )
        exit_candidates = self.get_candidates(PEER_FLAG_EXIT_ANY)
        if ctype == CIRCUIT_TYPE_IPV8:
            exit_candidates = (
                self.get_candidates(PEER_FLAG_EXIT_IPV8) or exit_candidates
            )
        ...
        if not required_exit:
            if ctype in [CIRCUIT_TYPE_DATA, CIRCUIT_TYPE_IPV8, CIRCUIT_TYPE_IP_SEEDER]:
                required_exit = (
                    random.choice(exit_candidates) if exit_candidates else None
                )
                # For introduction points we prefer exit nodes, but perhaps a relay peer would also suffice..
                if (
                    not required_exit
                    and relay_candidates
                    and ctype == CIRCUIT_TYPE_IP_SEEDER
                ):
        ...
        if goal_hops == 1 and required_exit:
            ...
            self.logger.info(
                "Look for a first hop that is not an exit node and is not used before"
            )
        ...
        self.circuits[circuit_id] = circuit = Circuit(
            circuit_id, goal_hops, ctype, required_exit, info_hash
        )
        self.send_initial_create(
            circuit,
            possible_first_hops,
    ...
    def send_initial_create(self, circuit, candidate_list, max_tries):
        ...
        (
            circuit.unverified_hop.dh_secret,
            circuit.unverified_hop.dh_first_part,
        ) = self.crypto.generate_diffie_secret()

        self.logger.info(
            "Adding first hop %s:%d to circuit %d",
            *(first_hop.address + (circuit.circuit_id,))
        )
        ...
        self.request_cache.add(
            RetryRequestCache(
                ...
                max_tries - 1,
                self.send_initial_create,
                self.settings.next_hop_timeout,
            )
        )
    ...
    async def remove_circuit(
        self, circuit_id, additional_info="", remove_now=False, destroy=False
    ):
        """
        ...
        self.logger.info(
            "Removing %s circuit %d %s",
            circuit_to_remove.ctype,
            circuit_id,
            additional_info,
        )
    ...
    async def remove_exit_socket(
        ...
        if exit_socket:
            ...
            if exit_socket.enabled:
                self.logger.info(
                    "Removing exit socket %d %s", circuit_id, additional_info
                )
    ...
    def destroy_relay(self, circuit_ids, reason=0, got_destroy_from=None):
        relays = {
            cid_from: (
                self.relay_from_to[cid_from].circuit_id,
                self.relay_from_to[cid_from].peer.address,
            )
            for cid_from in circuit_ids
            if cid_from in self.relay_from_to
        }

        if got_destroy_from and got_destroy_from not in relays.values():
            self.logger.error(
                "%s not allowed send destroy for circuit %s",
    ...
    def send_cell(self, candidates, message_type, payload, circuit_id=None):
        ...
        cell = CellPayload(
            circuit_id,
            pack("!B", message_id) + message,
            message_id in NO_CRYPTO_PACKETS,
        )
        try:
            cell.encrypt(
                self.crypto,
                self.circuits.get(circuit_id),
                self.relay_session_keys.get(circuit_id),
            )
    ...
    def relay_cell(self, cell):
        ...
        try:
            if next_relay.rendezvous_relay:
                cell.decrypt(
                    self.crypto,
                    relay_session_keys=self.relay_session_keys[cell.circuit_id],
                )
                cell.encrypt(
                    self.crypto,
                    relay_session_keys=self.relay_session_keys[next_relay.circuit_id],
                )
            else:
        ...
        packet = cell.to_bin(self._prefix)
        self.increase_bytes_sent(
            next_relay, self.send_packet([next_relay.peer], packet)
        )
    ...
    def _ours_on_created_extended(self, circuit, payload):
        ...
        try:
            shared_secret = self.crypto.verify_and_generate_shared_secret(
                hop.dh_secret, payload.key, payload.auth, hop.public_key.key.pk
            )
            hop.session_keys = self.crypto.generate_session_keys(shared_secret)

        except CryptoException:
            self.remove_circuit(
                circuit.circuit_id, "error while verifying shared secret, bailing out."
            )
            return
        ...
        if circuit.state == CIRCUIT_STATE_EXTENDING:
            ...
            _, candidate_list = decode(
                self.crypto.decrypt_str(
                    candidate_list_enc,
    ...
    def send_extend(self, circuit, candidate_list, max_tries):
        ...
        ignore_candidates = [
            self.crypto.key_to_bin(chop.public_key) for chop in circuit.hops
        ...
        if extend_hop_public_bin:
            extend_hop_public_key = self.crypto.key_from_public_bin(
                extend_hop_public_bin
            )
            ...
            if not become_exit or not circuit.required_exit:
                alt_candidates = [
                    c for c in candidate_list if c != extend_hop_public_bin
                ]
            else:
                alt_candidates = []
            self.request_cache.add(
            ...
            self.remove_circuit(
                circuit.circuit_id, "no candidates to extend, bailing out."
            )
    ...
    def extract_peer_flags(self, extra_bytes):
        ...
        payload = self.serializer.unpack_to_serializables(
            [ExtraIntroductionPayload], extra_bytes
        )[0]
    ...
    def create_introduction_request(self, socket_address, extra_bytes=b""):
        ...
        return super(TunnelCommunity, self).create_introduction_request(
    ...
    def create_introduction_response(
        ...
        extra_payload = ExtraIntroductionPayload(self.settings.peer_flags)
        extra_bytes = self.serializer.pack_multiple(extra_payload.to_pack_list())[0]
        return super(TunnelCommunity, self).create_introduction_response(
    ...
    def on_cell(self, source_address, data):
        ...
        if self.is_relay(circuit_id):
            ...
            self.logger.debug(
                "Relaying cell from circuit %d to %d", circuit_id, next_relay.circuit_id
            )
        ...
        try:
            cell.decrypt(
                self.crypto,
                circuit=circuit,
                relay_session_keys=self.relay_session_keys.get(circuit_id),
            )
        except CryptoException:
            if circuit:
            ...
            return
        self.logger.debug(
            "Got cell(%s) from circuit %d (sender %s, receiver %s)",
            cell.message[0],
            circuit_id,
            source_address,
            self.my_peer,
        ...
        if cell.plaintext and ord(cell.message[0:1]) not in NO_CRYPTO_PACKETS:
            self.logger.warning(
                "Dropping cell (only create/created can have plaintext flag set)"
            )
            return
        self.on_packet_from_circuit(
            source_address, cell.unwrap(self._prefix), circuit_id
        )
    ...
    def join_circuit(self, create_payload, previous_node_address):
        ...
        self.logger.info(
            "We joined circuit %d with neighbour %s", circuit_id, previous_node_address
        )

        shared_secret, key, auth = self.crypto.generate_diffie_shared_secret(
            create_payload.key
        )
        self.relay_session_keys[circuit_id] = self.crypto.generate_session_keys(
            shared_secret
        )

        peers_list = [
            peer
            for peer in self.get_candidates(PEER_FLAG_RELAY)
            if peer not in self.get_candidates(PEER_FLAG_EXIT_ANY)
        ][:4]
        ...
        self.request_cache.add(
            CreatedRequestCache(
                self, circuit_id, peer, peers_keys, self.settings.unstable_timeout
            )
        )
        self.exit_sockets[circuit_id] = TunnelExitSocket(circuit_id, peer, self)

        candidate_list_enc = self.crypto.encrypt_str(
            encode(list(peers_keys.keys())),
            *self.crypto.get_session_keys(
    ...
    async def on_create(self, source_address, payload, _):
        ...
        if self.request_cache.has("created", payload.circuit_id):
            self.logger.warning(
                "Already have a request for circuit %d", payload.circuit_id
            )
            return
        ...
        if result:
            ...
            self.logger.warning(
                "We're not joining circuit with ID %s", payload.circuit_id
            )
    ...
    def on_created(self, source_address, payload, _):
        ...
        if self.request_cache.has("create", payload.circuit_id):
            ...
            self.relay_from_to[request.to_circuit_id] = relay = RelayRoute(
                request.from_circuit_id, request.peer
            )
            self.relay_from_to[request.from_circuit_id] = RelayRoute(
                request.to_circuit_id, request.to_peer
            )
            self.relay_session_keys[request.to_circuit_id] = self.relay_session_keys[
                request.from_circuit_id
            ]
            ...
            self.logger.warning(
                "Received unexpected created for circuit %d", payload.circuit_id
            )
    ...
    async def on_extend(self, source_address, payload, _):
        ...
        if not self.request_cache.has("created", payload.circuit_id):
            self.logger.warning(
                "Received unexpected extend for circuit %d", payload.circuit_id
            )
            return
        ...
        if not (payload.node_addr or payload.node_public_key in request.candidates):
            self.logger.warning(
                "Node public key not in request candidates and no ip specified"
            )
            return
        ...
        if payload.node_public_key in request.candidates:
            ...
            extend_candidate = self.network.get_verified_by_public_key_bin(
        ...
        self.logger.info(
            "Extending circuit, got candidate with IP %s:%d from cache",
            *extend_candidate.address
        )

        self.request_cache.add(
            CreateRequestCache(
                self, to_circuit_id, circuit_id, candidate, extend_candidate
            )
        )
        ...
        self.send_cell(
            ...
            "create",
            CreatePayload(
                to_circuit_id, self.my_peer.public_key.key_to_bin(), payload.key
            ),
        )
    ...
    def on_extended(self, source_address, payload, _):
        if not self.request_cache.has("retry", payload.circuit_id):
            self.logger.warning(
                "Received unexpected extended for circuit %s", payload.circuit_id
            )
            return
    ...
    def on_data(self, sock_addr, data, _):
        ...
        if circuit and origin and sock_addr == circuit.peer.address:
            ...
            if DataChecker.could_be_ipv8(data):
                ...
                if isinstance(self.endpoint, TunnelEndpoint):
                    ...
                    self.logger.debug(
                        "Incoming packet meant for other community, dropping"
                    )
            else:
        ...
        # It is not our circuit so we got it from a relay, we need to EXIT it!
            ...
            self.logger.debug(
                "Data for circuit %d exiting tunnel (%s)", circuit_id, destination
            )
    ...
    def on_ping(self, source_address, payload, _):
        if not (
            payload.circuit_id in self.circuits
        ...
        self.send_cell(
            [source_address],
            "pong",
            PongPayload(payload.circuit_id, payload.identifier),
        )
    ...
    def do_ping(self, exclude=None):
        ...
        for circuit in list(self.circuits.values()):
            if (
                circuit.state in [CIRCUIT_STATE_READY, CIRCUIT_STATE_EXTENDING]
    ...
    def on_destroy(self, peer, payload):
        ...
        circuit_id = payload.circuit_id
        self.logger.info(
            "Got destroy from %s for circuit %s", source_address, circuit_id
        )
        ...
        if circuit_id in self.relay_from_to:
            ...
            circuit_id in self.exit_sockets
            and source_address == self.exit_sockets[circuit_id].peer.address
        ):
            ...
            and source_address == self.circuits[circuit_id].peer.address
        ):
    ...
    def exit_data(self, circuit_id, sock_addr, destination, data):
        ...
        if not (is_ipv8 or self.settings.peer_flags & PEER_FLAG_EXIT_ANY):
            self.logger.error(
                "Dropping data packets, refusing to be an exit node for data"
            )
            ...
            self.logger.error(
                "Dropping data packets, refusing to be an exit node for ipv8"
            )
            ...
            if not self.exit_sockets[circuit_id].enabled:
                ...
                if sock_addr[0] == self.exit_sockets[circuit_id].peer.address[0]:
                    ...
                    self.logger.error(
                        "Dropping outbound relayed packet: IP's are %s != %s",
                        str(sock_addr),
                        str(self.exit_sockets[circuit_id].peer.address),
                    )
            try:
