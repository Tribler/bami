...
class HiddenTunnelCommunity(TunnelCommunity):
    ...
    def join_swarm(self, info_hash, hops, callback=None, seeding=True):
        ...
        if info_hash in self.swarms:
            self.logger.warning(
                "Already part of hidden swarm %s, leaving existing",
                binascii.hexlify(info_hash),
            )
            self.leave_swarm(info_hash)

        self.swarms[info_hash] = Swarm(
            info_hash,
            hops,
    ...
    def leave_swarm(self, info_hash):
        ...
        if swarm:
            ...
            if not [
                s for s in self.swarms.values() if s != swarm and s.hops == swarm.hops
            ]:
                for circuit in self.find_circuits(hops=swarm.hops, state=None):
                    self.remove_circuit(
                        circuit.circuit_id, "leaving hidden swarm", destroy=True
                    )
            # Remove e2e circuits
            for rp_circuit, _ in swarm.connections.values():
                self.remove_circuit(
                    rp_circuit.circuit_id, "leaving hidden swarm", destroy=True
                )
        # Remove introduction points
        for ip_circuit in self.circuits.values():
            if (
                ip_circuit.info_hash == info_hash
                and ip_circuit.ctype == CIRCUIT_TYPE_IP_SEEDER
            ):
                self.remove_circuit(
                    ip_circuit.circuit_id, "leaving hidden swarm", destroy=True
                )
    ...
    async def estimate_swarm_size(self, info_hash, hops=1, max_requests=10):
        ...
        while len(tried) < max_requests:
            ...
            ips = random.sample(
                not_tried, min(len(not_tried), max_requests - len(tried))
            )
            responses = await gather(
                *[swarm.lookup(ip) for ip in ips], return_errors=True
            )

            # Collect responses
            all_ += sum(
                [result for result in responses if not isinstance(result, Exception)],
                [],
            )
            tried |= set(ips)
    ...
    async def do_peer_discovery(self):
        ...
        for info_hash, swarm in list(self.swarms.items()):
            if (
                not swarm.seeding
                and swarm.last_lookup + self.settings.swarm_lookup_interval <= now
                ...
                try:
                    ...
                    self.logger.info(
                        "Failed to do peer discovery for swarm %s",
                        binascii.hexlify(info_hash),
                    )
                    continue
                ...
                if ips is None:
                    self.logger.info(
                        "Skipping peer discovery for swarm %s",
                        binascii.hexlify(info_hash),
                    )
                    continue
                self.logger.info(
                    "Found %d/%d peer(s) for swarm %s",
    ...
    def do_ping(self, exclude=None):
        ...
        exclude += [
            ...
            if (c.ctype == CIRCUIT_TYPE_RP_SEEDER)
            or (c.ctype == CIRCUIT_TYPE_RP_DOWNLOADER and not c.e2e)
        ]
    ...
    def remove_circuit(
        ...
        return super(HiddenTunnelCommunity, self).remove_circuit(
            circuit_id, additional_info, remove_now, destroy
        )

    def remove_exit_socket(
        self, circuit_id, additional_info="", remove_now=False, destroy=False
    ):
                ...
                if pex:
                    ...
                    if pex.done:
                        ...
                        self.ipv8.strategies = [
                            t for t in self.ipv8.strategies if t[0].overlay != pex
                        ]
        ...
        return super(HiddenTunnelCommunity, self).remove_exit_socket(
            circuit_id, additional_info, remove_now, destroy
        )
    ...
    def get_max_time(self, circuit_id):
        if (
            ...
            and self.circuits[circuit_id].ctype == CIRCUIT_TYPE_IP_SEEDER
        ):
            return self.settings.max_time_ip
        if circuit_id in self.exit_sockets and circuit_id in [
            c.circuit_id for c, _ in self.intro_point_for.values()
        ]:
            return self.settings.max_time_ip
    ...
    def tunnel_data(self, circuit, destination, message_type, payload):
        ...
        packet = self._ez_pack(
            self._prefix, message_id, [payload.to_pack_list()], False
        )
        pre = ("0.0.0.0", 0)
        post = ("0.0.0.0", 0)
    ...
    def select_circuit(self, destination, hops):
        ...
        if destination and destination[1] == CIRCUIT_ID_PORT:
            ...
            if (
                ...
                and circuit.ctype == CIRCUIT_TYPE_RP_DOWNLOADER
            ):
                return circuit
    ...
    async def on_peers_request(self, source_address, payload, circuit_id=None):
        ...
        self.logger.info(
            "Doing hidden seeders lookup for info_hash %s", binascii.hexlify(info_hash)
        )
        if info_hash in self.pex:
            ...
            self.logger.warning(
                "Received a peers-request over circuit %d, but unable to do a DHT lookup",
                circuit_id,
            )
        else:
            self.logger.warning(
                "Received a peers-request over the socket, but unable to do a PEX lookup"
            )
    ...
    def send_peers_response(self, target_addr, request, intro_points, circuit_id):
        result = encode(
            [
                (
                    ip.peer.address,
                    ip.peer.public_key.key_to_bin(),
                    ip.seeder_pk,
                    ip.source,
                )
                for ip in intro_points[:7]
            ]
        )
        payload = PeersResponsePayload(
            request.circuit_id, request.identifier, request.info_hash, result
        )
    ...
    def on_peers_response(self, source_address, payload, circuit_id):
        ...
        ips = [
            IntroductionPoint(Peer(ip_pk, address=address), seeder_pk, source)
            for address, ip_pk, seeder_pk, source in peers
    ...
    def create_e2e(self, info_hash, intro_point):
        ...
        self.logger.info(
            "Creating e2e circuit for introduction point %s", intro_point.peer
        )
        cache = self.request_cache.add(
            E2ERequestCache(self, info_hash, hop, intro_point)
        )
        self.tunnel_data(
            circuit,
            intro_point.peer.address,
            u"create-e2e",
            CreateE2EPayload(
                cache.number, info_hash, hop.node_public_key, hop.dh_first_part
            ),
        )
    ...
    async def on_create_e2e(self, source_address, payload, circuit_id=None):
        ...
        if circuit_id is None:
            if payload.node_public_key in self.intro_point_for:
                self.logger.info(
                    "On create-e2e: forwarding message because received over socket"
                )
                ...
                self.logger.info(
                    "On create-e2e: dropping message for unknown seeder key %s",
    ...
    def create_created_e2e(self, rendezvous_point, source_address, payload, circuit_id):
        ...
        shared_secret, Y, AUTH = self.crypto.generate_diffie_shared_secret(
            payload.key, key
        )
        rendezvous_point.circuit.hs_session_keys = self.crypto.generate_session_keys(
            shared_secret
        )
        rp_info_enc = self.crypto.encrypt_str(
            encode((rendezvous_point.rp_info, rendezvous_point.cookie)),
            *self.crypto.get_session_keys(
                rendezvous_point.circuit.hs_session_keys, EXIT_NODE
            )
        )
    ...
    async def on_created_e2e(self, source_address, payload, circuit_id):
        ...
        shared_secret = self.crypto.verify_and_generate_shared_secret(
            cache.hop.dh_secret, payload.key, payload.auth, cache.hop.public_key.key.pk
        )
        ...
        circuit = self.create_circuit_for_infohash(
            cache.info_hash, CIRCUIT_TYPE_RP_DOWNLOADER, required_exit=required_exit
        )
        if circuit:
            self.swarms[cache.info_hash].add_connection(circuit, cache.intro_point)
            if await circuit.ready:
                cache = self.request_cache.add(
                    LinkRequestCache(self, circuit, cache.info_hash, session_keys)
                )
                self.send_cell(
                    [circuit.peer],
                    u"link-e2e",
                    LinkE2EPayload(circuit.circuit_id, cache.number, cookie),
                )
    ...
    def on_link_e2e(self, source_address, payload, circuit_id):
        ...
        self.relay_from_to[circuit.circuit_id] = RelayRoute(
            relay_circuit.circuit_id, relay_circuit.peer, True
        )
        self.relay_from_to[relay_circuit.circuit_id] = RelayRoute(
            circuit.circuit_id, circuit.peer, True
        )

        self.send_cell(
            [source_address],
            u"linked-e2e",
            LinkedE2EPayload(circuit.circuit_id, payload.identifier),
        )
    ...
    def on_linked_e2e(self, source_address, payload, circuit_id):
        ...
        if callback:
            result = callback(
                (self.circuit_id_to_ip(circuit.circuit_id), CIRCUIT_ID_PORT)
            )
            if iscoroutine(result):
                self.register_anonymous_task("e2e_callback", result)
        else:
            self.logger.error(
                "On linked e2e: could not find download for %s!", cache.info_hash
            )
    ...
    async def create_introduction_point(self, info_hash, required_ip=None):
        ...
        if info_hash not in self.swarms:
            ...
            self.logger.warning(
                "Cannot create introduction point for swarm that is not seeding"
            )
            return

        circuit = self.create_circuit_for_infohash(
            info_hash, CIRCUIT_TYPE_IP_SEEDER, required_exit=required_ip
        )
        ...
        if circuit and await circuit.ready:
            ...
            self.send_cell(
                [circuit.peer],
                "establish-intro",
                EstablishIntroPayload(circuit_id, cache.number, info_hash, seed_pk),
            )
    ...
    def on_establish_intro(self, source_address, payload, circuit_id):
        if payload.public_key in self.intro_point_for:
            self.logger.warning(
                "Already have an introduction point for %s",
                binascii.hexlify(payload.public_key),
            )
            return

        self.logger.info(
            "Established introduction point for %s",
            binascii.hexlify(payload.public_key),
        )
        ...
        if not self.ipv8:
            self.logger.error(
                "No IPv8 service object available, cannot start PEXCommunity"
            )
        elif payload.info_hash not in self.pex:
            community = PexCommunity(
                self.my_peer, self.endpoint, Network(), info_hash=payload.info_hash
            )
        ...
        self.dht_announce(
            payload.info_hash,
            IntroductionPoint(
                Peer(self.my_peer.key, self.my_estimated_wan), payload.public_key
            ),
        )
    ...
    async def create_rendezvous_point(self, info_hash):
        ...
        if circuit and await circuit.ready:
            ...
            self.send_cell(
                ...
                EstablishRendezvousPayload(circuit.circuit_id, cache.number, rp.cookie),
            )
            return rp
