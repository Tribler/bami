...
class OrderedSet(set):
    ...
    def __init__(self, decorated: Union[List[Union[int, str]], List[bytes]]) -> None:
        super(OrderedSet, self).__init__(decorated)
        self.decorated = decorated

    def __iter__(self) -> Iterator[Union[int, str, bytes]]:
...
class Sortable(object):
    ...
    def compare(self, other: Sortable) -> int:
        """
        ...
        if other.type == self.type:
            ...
            if self.type == SortableTypeEnum.STRING:
                ...
                my_ordinals = (
                    ...
                    else [ord(c) for c in self.value]
                )
                other_ordinals = (
                    other.value
                    if isinstance(other.value, bytes)
                    else [ord(c) for c in other.value]
                )
            else:
...
def sortable_sort(data: Any) -> Any:
    """
