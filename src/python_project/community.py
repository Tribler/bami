...
BOOTSTRAP_TIMEOUT = (
    30.0  # Timeout before we bootstrap again (bootstrap kills performance)
)
DEFAULT_MAX_PEERS: int = 30
...
class Community(EZPackOverlay):
    ...
    def __init__(
        ...
        self._prefix: bytes = b"\x00" + self.version + self.master_peer.mid
        self.endpoint.remove_listener(self)
        self.endpoint.add_prefix_listener(self, self._prefix)
        self.logger.debug(
            "Launching %s with prefix %s.",
            self.__class__.__name__,
            hexlify(self._prefix),
        )
        ...
        if anonymize:
            if isinstance(self.endpoint, TunnelEndpoint):
                ...
                self.logger.warning(
                    "Cannot anonymize community traffic without TunnelEndpoint"
                )
    ...
    def add_message_handler(self, msg_num, callback):
        ...
        if not isinstance(msg_num, int):
            if not hasattr(msg_num, "msg_id"):
                raise RuntimeError(
                    "Attempted to add a handler for Payload %s, which does not specify a msg_id!"
                    % msg_num
                )
            msg_num = msg_num.msg_id
        if msg_num < 0 or msg_num > 255:
            raise RuntimeError(
                "Attempted to add a handler for message number %d, which is not a byte!"
                % msg_num
            )
        if chr(msg_num) in self.decode_map:
            raise RuntimeError(
                "Attempted to add a handler for message number %d, already mapped to %s!"
    ...
    def on_deprecated_message(self, source_address, data):
        self.logger.warning(
            "Received deprecated message: %s from (%s, %d)",
            self.deprecated_message_names[data[22]],
            *source_address
        )
    ...
    def bootstrap(self):
        if time() - self.last_bootstrap < BOOTSTRAP_TIMEOUT:
            return
        self.logger.debug(
            "Bootstrapping %s, current peers %d",
            self.__class__.__name__,
            len(self.get_peers()),
        )
    ...
    def create_introduction_response(
        ...
        if introduced:
            packet = self.create_puncture_request(
                lan_socket_address, socket_address, identifier, prefix=prefix
            )
            self.endpoint.send(
                introduction_wan
                if introduction_lan == ("0.0.0.0", 0)
                else introduction_lan,
                packet,
            )
    ...
    def create_puncture_request(self, lan_walker, wan_walker, identifier, prefix=None):
        ...
        payload = PunctureRequestPayload(
            lan_walker, wan_walker, identifier
        ).to_pack_list()
        dist = GlobalTimeDistributionPayload(global_time).to_pack_list()
    ...
    def on_introduction_request(self, peer, dist, payload):
        if self.max_peers >= 0 and len(self.get_peers()) > self.max_peers:
            self.logger.info(
                "Dropping introduction request from (%s, %d): too many peers!",
                peer.address[0],
                peer.address[1],
            )
            return
    ...
    def on_introduction_response(self, peer, dist, payload):
        ...
        if (
            payload.wan_introduction_address != ("0.0.0.0", 0)
            ...
            if payload.lan_introduction_address != ("0.0.0.0", 0):
                self.network.discover_address(
                    peer, payload.lan_introduction_address, self.master_peer.mid
                )
            self.network.discover_address(
                peer, payload.wan_introduction_address, self.master_peer.mid
            )
        elif (
            ...
            self.network.discover_address(
                peer,
                (self.my_estimated_lan[0], payload.wan_introduction_address[1]),
    ...
    def on_puncture_request(self, source_address, dist, payload):
        ...
        packet = self.create_puncture(
            self.my_estimated_lan, payload.wan_walker_address, payload.identifier
        )
            ...
            try:
                ...
                if iscoroutine(result):
                    self.register_anonymous_task(
                        "on_packet", ensure_future(result), ignore=(Exception,)
                    )
            except Exception:
                self.logger.error(
    ...
    def get_peer_by_pub_key(self, pub_key):
        return self.network.get_service_peer_by_public_key_bin(
            pub_key, self.master_peer.mid
        )
