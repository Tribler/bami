...
class DHTEndpoint(BaseEndpoint):
    ...
    async def get_statistics(self, _):
        ...
        stats = {
            "node_id": hexlify(self.dht.my_node_id).decode("utf-8"),
        ...
        if isinstance(self.dht, DHTDiscoveryCommunity):
            stats.update(
                {
                    "num_peers_in_store": {
                        hexlify(key).decode("utf-8"): len(peers)
        ...
        responses={
            200: {
                "schema": schema(
                    DHTPeerResponse={
                        "peers": [
                            schema(DHTPeer={"public_key": String, "address": String})
                        ]
                    }
                )
            },
        ...
        return Response(
            {
                "peers": [
                    {
                        "public_key": b64encode(node.public_key.key_to_bin()).decode(
                            "utf-8"
                        ),
    ...
    @docs(
        ...
        responses={
            200: {
                "schema": schema(DHTStoredValuesResponse={"values": [DHTValueSchema]})
            },
            400: {
                "schema": DefaultResponseSchema,
                "examples": {
    ...
    @docs(
        ...
        responses={
            200: {"schema": schema(DHTValuesResponse={"values": [DHTValueSchema]})},
            400: {
                "schema": DefaultResponseSchema,
                "examples": {
                    "DHT disabled": {
    ...
    @json_schema(schema(DHTStoreRequest={"value*": String}))
    ...
    @docs(
        ...
        responses={
            200: {
                "schema": schema(DHTBlockResponse={"block": (String, "Bencoded block")})
            },
            400: {
                "schema": DefaultResponseSchema,
                "examples": {
    ...
    async def get_block(self, request):
        ...
        if "public_key" not in parameters:
            return Response(
                {"error": "Must specify the peer's public key"}, status=HTTP_BAD_REQUEST
            )
...
class DHTBlockPublisher(BlockListener):
    ...
    def __init__(self, dht, trustchain):
        ...
        self._hashed_dht_key = sha1(
            self.trustchain.my_peer.public_key.key_to_bin() + self.KEY_SUFFIX
        ).digest()

        self.trustchain.register_task(
            "block_maintenance",
    ...
    def received_block(self, block):
        ...
        if (
            block
            and block.public_key == self.trustchain.my_peer.public_key.key_to_bin()
        ):
    ...
    async def publish_latest_block(self):
        ...
        """
        block = self.trustchain.persistence.get_latest(
    ...
    async def publish_block(self, block, republish=False):
        ...
        actual_version = max(
            self.block_version if not republish else (self.block_version - 1), 0
        )
        ...
        if total_chunks > 0:
            ...
            while chunk_attempt <= self.ATTEMPT_LIMIT:
                ...
                blob_chunk = self.serializer.pack_multiple(
                    DHTBlockPayload(
                        signature, actual_version, chunk_idx, total_chunks, chunk
                    ).to_pack_list()
                )
            ...
            if chunk_attempt > self.ATTEMPT_LIMIT:
                logging.error(
                    "Publishing latest block failed after %d attempts on chunk %d",
                    self.ATTEMPT_LIMIT,
                    chunk_idx,
                )
    ...
    async def retrieve_block(self, raw_public_key):
        ...
        while new_chunks:
            ...
            chunk_dict, max_version, counts = self.reconstruct_all_blocks(
                [x[0] for x in new_chunks], target_public_key, chunk_dict, counts
            )
    ...
    def reconstruct_all_blocks(
        self, new_chunks, public_key, chunk_dict=None, counts=None
    ):
        """
        ...
        """
        assert (chunk_dict and counts) or (not chunk_dict and not counts), (
            "Must either provide both new_blocks " "and counter dicts or None for both"
        )
        ...
        for entry in new_chunks:
            try:
                package = self.serializer.unpack_to_serializables(
                    [DHTBlockPayload,], entry
                )[0]
                ...
                if public_key.verify(package.signature, pre_signed_content):
                    ...
                    if not chunk_dict[package.version][package.block_position]:
                        chunk_dict[package.version][
                            package.block_position
                        ] = package.payload
                        counts[package.version] -= 1
            except PackError:
                logging.error(
                    "PackError: Found a clandestine entry in the DHT when reconstructing TC blocks: %s",
                    entry,
                )
