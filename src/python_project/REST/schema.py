...
class DefaultResponseSchema(Schema):
    ...
    error = String(
        description="Optional field describing any failures that may have occurred",
        required=False,
    )
...
def schema(**kwargs):
    ...
    for k, v in spec.items():
        if isinstance(v, SchemaMeta):
            ...
            schema_dict[k] = (
                List(Nested(v[0])) if isinstance(v[0], SchemaMeta) else List(v[0])
            )
        else:
            d = None
            if isinstance(v, tuple):
                v, d = v
            schema_dict[k.rstrip("*")] = (
                v.__call__(description=d, required=k.endswith("*"))
                if callable(v)
                else v
            )
