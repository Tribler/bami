...
class OverlaysEndpoint(BaseEndpoint):
    ...
    def initialize(self, session):
        ...
        self.statistics_supported = isinstance(
            session.endpoint, StatisticsEndpoint
    ...
    @docs(
        ...
        responses={
            200: {"schema": schema(OverlayResponse={"overlays": [OverlaySchema]})}
        },
    )
    async def get_overlays(self, _):
        ...
        for overlay in self.session.overlays:
            ...
            statistics = (
                self.session.endpoint.get_aggregate_statistics(overlay.get_prefix())
                if isinstance(self.session.endpoint, StatisticsEndpoint)
                else {}
            )
            overlay_stats.append(
                {
                    "master_peer": hexlify(
                        overlay.master_peer.public_key.key_to_bin()
                    ).decode("utf-8"),
                    ...
                    "peers": [
                        {
                            "ip": peer.address[0],
                            "port": peer.address[1],
                            "public_key": hexlify(peer.public_key.key_to_bin()).decode(
                                "utf-8"
                            ),
                        }
                        for peer in peers
        ...
        responses={
            200: {
                "schema": schema(
                    StatisticsResponse={
                        "statistics": List(
                            Dict(keys=String, values=Nested(OverlayStatisticsSchema))
                        ),
                    }
                ),
                "examples": {
    ...
    async def get_statistics(self, _):
        ...
        for overlay in self.session.overlays:
            statistics = (
                self.session.endpoint.get_statistics(overlay.get_prefix())
                if self.statistics_supported
                else {}
            )
            overlay_stats.append(
                {
                    overlay.__class__.__name__: self.statistics_by_name(
                        statistics, overlay
                    )
                }
            )
    ...
    def statistics_by_name(self, statistics, overlay):
        ...
        for message_id, network_stats in statistics.items():
            if overlay.decode_map.get(chr(message_id)):
                mapped_name = (
                    str(message_id) + ":" + overlay.decode_map[chr(message_id)].__name__
                )
            else:
    ...
    async def enable_statistics(self, request):
        if not self.statistics_supported:
            return Response(
                {"success": False, "error": "StatisticsEndpoint is not enabled."},
        ...
        if "enable" not in args or not args["enable"]:
            return Response(
                {"success": False, "error": "Parameter 'enable' is required"},
        ...
        self.enable_overlay_statistics(
            enable=enable, class_name=overlay_name, all_overlays=all_overlays
        )
        return Response({"success": True})

    def enable_overlay_statistics(
        self, enable=False, class_name=None, all_overlays=False
    ):
        if all_overlays:
            for overlay in self.session.overlays:
