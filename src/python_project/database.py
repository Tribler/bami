...
class IgnoreCommits(Exception):
    ...
    def __init__(self):
        super(IgnoreCommits, self).__init__(
            "Ignore all commits made within __enter__ and __exit__"
        )
...
class Database(metaclass=ABCMeta):
    ...
    def open(self, initial_statements=True, prepare_visioning=True):
        ...
        self._assert(
            self._connection is None, "Database.open() has already been called"
        )

        self._logger.debug("open database [%s]", self._file_path)
        if (not self._file_path.startswith(":")) and (
            not os.path.isfile(self._file_path)
        ):
    ...
    def _initial_statements(self):
        ...
        journal_mode = cast_to_unicode(
            next(self._cursor.execute(u"PRAGMA journal_mode"))[0]
        ).upper()
        synchronous = cast_to_unicode(
            next(self._cursor.execute(u"PRAGMA synchronous"))[0]
        ).upper()
        ...
        if page_size < 8192:
            self._logger.debug(
                "PRAGMA page_size = 8192 (previously: %s) [%s]",
                page_size,
                self._file_path,
            )
            ...
            self._logger.debug(
                "PRAGMA page_size = %s (no change) [%s]", page_size, self._file_path
            )
        ...
        if not (journal_mode == u"WAL" or self._file_path == u":memory:"):
            self._logger.debug(
                "PRAGMA journal_mode = WAL (previously: %s) [%s]",
                journal_mode,
                self._file_path,
            )
            ...
            self._logger.debug(
                "PRAGMA journal_mode = %s (no change) [%s]",
                journal_mode,
                self._file_path,
            )
        ...
        if synchronous not in (u"NORMAL", u"1"):
            self._logger.debug(
                "PRAGMA synchronous = NORMAL (previously: %s) [%s]",
                synchronous,
                self._file_path,
            )
            ...
            self._logger.debug(
                "PRAGMA synchronous = %s (no change) [%s]", synchronous, self._file_path
            )
    ...
    def _prepare_version(self):
        ...
        self._assert(
            isinstance(self._database_version, int),
            "expected databse version to be int or long, but was type %s"
            % str(type(self._database_version)),
        )
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if exc_type is None:
            ...
            if pending_commits > 1:
                self._logger.debug(
                    "performing %d pending commits [%s]",
                    pending_commits - 1,
                    self._file_path,
                )
                self.commit()
            ...
            self._logger.debug(
                "enabling commit without committing now [%s]", self._file_path
            )
            return True
    ...
    def execute(self, statement, bindings=(), get_lastrowid=False, fetch_all=True):
        ...
        """
        self._logger.log(
            logging.NOTSET, "%s <-- %s [%s]", statement, bindings, self._file_path
        )
    ...
    def executescript(self, statements, fetch_all=True):
        ...
        self._assert(
            isinstance(statements, str), "The SQL statement must be given in unicode"
        )
    ...
    def commit(self, exiting=False):
        ...
        self._assert(
            not (exiting and self._pending_commits),
            "No pending commits should be present when exiting",
        )
