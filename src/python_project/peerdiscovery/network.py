...
class Network(object):
    ...
    def discover_address(self, peer, address, service=None):
        ...
        with self.graph_lock:
            if (address not in self._all_addresses) or (
                self._all_addresses[address][0]
                not in [p.mid for p in self.verified_peers]
            ):
    ...
    def get_service_peer_by_public_key_bin(self, public_key_bin, service_id):
        with self.graph_lock:
            ...
            if (
                peer
                and peer.mid in self.services_per_peer
                and service_id in self.services_per_peer[peer.mid]
            ):
                return peer
    ...
    def get_peers_for_service(self, service_id):
        ...
        if service_cache is None:
            ...
            out = [
                peer
                for peer in service_cache
                if peer in self.verified_peers
                and service_id in self.services_per_peer.get(peer.mid, [])
            ]
    ...
    def get_walkable_addresses(self, service_id=None):
        ...
        with self.graph_lock:
            known = (
                self.get_peers_for_service(service_id)
                if service_id
                else self.verified_peers
            )
                ...
                for p in self.verified_peers:
                    if p.address == address:
                        ...
                        if len(self.reverse_ip_lookup) > self.reverse_ip_cache_size:
                            self.reverse_ip_lookup.popitem(
                                False
                            )  # Pop the oldest cache entry
                        break
            else:
        ...
        with self.graph_lock:
            ...
            for peer in self.verified_peers:
                if peer.address and peer.address != ("0.0.0.0", 0):
                    out += inet_aton(
                        cast_to_chr(peer.address[0])
                        if isinstance(peer.address[0], bytes)
                        else peer.address[0]
                    ) + pack(">H", peer.address[1])
            return out
