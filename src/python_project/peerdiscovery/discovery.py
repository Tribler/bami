...
class RandomWalk(DiscoveryStrategy):
    ...
    def __init__(
        self, overlay, timeout=3.0, window_size=5, reset_chance=50, target_interval=0
    ):
        """
    ...
    def take_step(self):
        ...
        with self.walk_lock:
            ...
            if (
                ...
                and self.last_step + self.target_interval >= time()
            ):
                return
            ...
            if (
                ...
                and len(self.intro_timeouts) >= self.window_size
            ):
                return
            # Take step
...
class EdgeWalk(DiscoveryStrategy):
    ...
    def take_step(self):
        ...
        with self.walk_lock:
            if (
                not self._neighborhood
                or len(self._neighborhood) < self.neighborhood_size
            ):
                ...
                for peer in self.overlay.get_walkable_addresses()[
                ...
                if waiting_root:
                    ...
                    for root in self.under_construction:
                        ...
                        introductions = []
                        for intro in self.overlay.network.get_introductions_from(
                            last_verified
                        ):
                            verified = self.overlay.network.get_verified_by_address(
                                intro
                            )
                            if verified:
                        ...
                        if introductions:
                            ...
                            if len(self.under_construction[root]) == self.edge_length:
                                ...
                                self.complete_edges.append(
                                    self.under_construction[root]
                                )
                            ...
                            self.last_edge_responses[root] + self.edge_timeout < time()
                        ):
