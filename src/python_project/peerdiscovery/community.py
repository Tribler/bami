...
class PeriodicSimilarity(DiscoveryStrategy):
    ...
    def take_step(self):
        ...
        self.last_step = now
        with self.walk_lock:
            self.overlay.send_similarity_request(
                sample(self.overlay.network.verified_peers, 1)[0].address
            )


class PingRequestCache(NumberCache):
    def __init__(self, request_cache, identifier, peer, start_time):
        super(PingRequestCache, self).__init__(
            request_cache, u"discoverypingcache", identifier
        )
        self.peer = peer
...
class DiscoveryCommunity(Community):
    ...
    master_peer = Peer(
        unhexlify(
            "3081a7301006072a8648ce3d020106052b81040027038192000403b3ab059ced"
    ...
    def __init__(
        self, my_peer, endpoint, network, max_peers=DEFAULT_MAX_PEERS, anonymize=False
    ):
        super(DiscoveryCommunity, self).__init__(
            my_peer, endpoint, network, max_peers=max_peers, anonymize=anonymize
        )
    ...
    def on_introduction_request(self, source_address, data):
        ...
        try:
            auth, dist, payload = self._ez_unpack_auth(
                DiscoveryIntroductionRequestPayload, data
            )
    ...
    def send_similarity_request(self, address):
        my_peer_set = set(
            [overlay.my_peer for overlay in self.network.service_overlays.values()]
        )
    ...
    def on_similarity_response(self, node, dist, payload):
        if (
            self.max_peers >= 0
            and len(self.get_peers()) > self.max_peers
            and node not in self.network.verified_peers
        ):
            self.logger.info(
                "Dropping similarity response from (%s, %d): too many peers!",
                node.address[0],
                node.address[1],
            )
            return
    ...
    def get_my_overlays(self, peer):
        return [
            service_id
            for service_id, overlay in self.network.service_overlays.items()
    ...
    def create_similarity_response(self, identifier, peer):
        ...
        payload = SimilarityResponsePayload(
            identifier, self.get_my_overlays(peer), []
        ).to_pack_list()
        auth = BinMemberAuthenticationPayload(
            peer.public_key.key_to_bin()
        ).to_pack_list()
    ...
    def send_ping(self, peer):
        ...
        self.request_cache.add(
            PingRequestCache(self.request_cache, global_time, peer, time())
        )
