class BlockCache(object):
    ...
    def __init__(self, database, public_key):
        ...
        self.blocks = (
            {}
        )  # Dictionary to hold known blocks (keys: sequence number, value: block)
        self.linked_blocks = (
            {}
        )  # Dictionary to hold linked blocks (keys: sequence number, value: block)
        ...
        if missing:
            ...
            db_result = list(
                self.database.execute(
                    query, (self.public_key, self.public_key), fetch_all=True
                )
            )
            blocks = [
                self.database.get_block_class(
                    db_item[0]
                    if isinstance(db_item[0], bytes)
                    else str(db_item[0]).encode("utf-8")
                )(db_item)
