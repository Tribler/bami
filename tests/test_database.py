...
class TestDBManager:
    ...
    def test_get_block_blob(self, monkeypatch, std_vals):
        ...
        monkeypatch.setattr(
            MockBlockStore,
            "get_block_by_hash",
            lambda _, blob_hash: self.block_blob if blob_hash == self.test_hash else None,
        )

        assert (
                self.dbms.get_block_blob_by_dot(self.chain_id, self.block_dot) == self.block_blob
        )
    ...
    def test_add_notify_block(self, monkeypatch, std_vals):
        ...
        monkeypatch.setattr(
            MockChain,
            "add_block",
            lambda _, parsed_block: ['dot1', 'dot2'] if parsed_block == self.test_block else None,
        )

        def chain_dots_tester(chain_dots):
            assert len(chain_dots) == 2
            assert list(chain_dots.keys())[0] == self.test_block.public_key
            assert list(chain_dots.keys())[1] == self.test_block.com_id
            assert list(chain_dots.values()) == [['dot1', 'dot2']]*2
