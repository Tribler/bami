...
class TestAttestationEndpoint(RESTTestBase):
    ...
    async def make_outstanding(self, node):
        ...
        return await self.make_request(
            node, "attestation", "get", {"type": "outstanding"}
        )
    ...
    async def make_verification_output(self, node):
        ...
        """
        return await self.make_request(
            node, "attestation", "get", {"type": "verification_output"}
        )
    ...
    async def make_attributes(self, node):
        ...
        """
        return await self.make_request(
            node, "attestation", "get", {"type": "attributes"}
        )
    ...
    async def make_dht_block(self, node, public_key):
        ...
        """
        return await self.make_request(
            node, "attestation", "get", {"public_key": public_key}
        )
    ...
    async def make_drop_identity(self, node):
        ...
        """
        return await self.make_request(
            node, "attestation", "get", {"type": "drop_identity"}
        )
    ...
    async def make_outstanding_verify(self, node):
        ...
        """
        return await self.make_request(
            node, "attestation", "get", {"type": "outstanding_verify"}
        )
    ...
    async def make_allow_verify(self, node, attribute_name, mid):
        ...
        return await self.make_request(
            ...
            "attestation",
            "post",
            {"type": "allow_verify", "attribute_name": attribute_name, "mid": mid},
        )
    ...
    async def create_attestation_request(self, node, attribute_name, metadata=None):
        ...
        for mid in peer_list:
            await self.make_attestation_request(
                node, attribute_name, mid, metadata=metadata
            )
    ...
    async def attest_all_outstanding_requests(
        self, node, attribute_name, attribute_value
    ):
        """
        ...
        self.assertFalse(
            outstanding_requests == [], "Something went wrong, no request was received."
        )
        ...
        for outstanding_request in outstanding_requests:
            ...
            attribute_value = (
                b64encode(attribute_value).decode("utf-8")
                if isinstance(attribute_value, bytes)
                else attribute_value
            )
            response = await self.make_attest(
                node, attribute_name, attribute_value, mid
            )
            responses.append(response)

        return outstanding_requests, responses

    async def verify_all_attestations(
        self, node, peer_mids, attribute_hash, attribute_values
    ):
        """
        ...
        for mid in peer_mids:
            ...
            intermediary_response = await self.make_verify(
                node, attribute_hash, attribute_values, mid
            )
    ...
    async def test_get_peers_request(self):
        ...
        self.assertTrue(
            any(x in other_peer_mids for x in result), "Could not find the second peer."
        )
    ...
    async def test_get_outstanding_requests(self):
        ...
        self.assertTrue(
            any((x[0] == mid and x[1] == "QR" for x in result)),
    ...
    async def test_get_verification_output(self):
        ...
        verification_responses = await self.verify_all_attestations(
            self.nodes[0], [self.nodes[1].my_peer.mid], attribute_hash, "YXNk,YXNkMg=="
        )
        self.assertTrue(
            all("success" in x and x["success"] for x in verification_responses),
        ...
        self.assertIsNotNone(
            ...
            "Could not retrieve any outstanding verifications",
        )
        ...
        self.assertTrue(
            [["YXNk", 0.0], ["YXNkMg==", 0.0]] in verification_output.values(),
    ...
    async def test_trustchain_recent(self):
        ...
        self.assertEqual(
            block1["transaction"]["metadata"], block2["transaction"]["metadata"]
        )
    ...
    async def test_get_outstanding_verify(self):
        ...
        self.assertTrue(
            any(
                x in result
                for x in [b64encode(self.nodes[0].my_peer.mid).decode("utf-8")]
            ),
    ...
    async def test_get_attributes(self):
        ...
        attributes = await self.make_attributes(self.nodes[1])
        self.assertTrue(
            attributes[0][0] == "QR" and attributes[0][1] != "",
    ...
    async def test_get_drop_identity(self):
        ...
        self.assertFalse(
            outstanding_requests == [], "The attestation requests were not received."
        )

        # Ensure that no block/attestation exists
        attributes = await self.make_attributes(self.nodes[1])
        self.assertEqual(
            attributes, [], "Something's wrong, there shouldn't be any blocks."
        )
        ...
        attributes = await self.make_attributes(self.nodes[1])
        self.assertNotEqual(
            attributes, [], "Something's wrong, the attribute list should be non-empty."
        )
        ...
        self.assertIn(
            "success",
            result,
            "The identity could not be dropped. Success parameter not in response.",
        )
        self.assertTrue(
            result["success"], "The identity could not be dropped, not successful."
        )
        ...
        self.assertEqual(
            result,
            [],
            "The identity could not be dropped. Outstanding requests still remaining.",
        )
    ...
    async def test_post_attestation_request(self):
        ...
        self.assertEqual(
            ...
            [],
            "Something went wrong, there should be no outstanding requests.",
        )
    ...
    async def test_post_attest(self):
        ...
        self.assertTrue(
            len(attributes) == 0,
            "There mustn't already be any attestations in the other peer.",
        )

        responses = await self.attest_all_outstanding_requests(
            self.nodes[0], "QR", b"data"
        )
        request_responses = [response for response in responses[1]]
        self.assertTrue(
            all("success" in x and x["success"] for x in request_responses),
            "Something went wrong, not all responses were successful.",
        )

        attributes = await self.make_attributes(self.nodes[1])
        self.assertTrue(
            len(attributes) == 1, "There should only be one attestation in the DB."
        )
        self.assertTrue(
            attributes[0][0] == "QR",
            "Expected attestation for %s, got it for " "%s" % ("QR", attributes[0][0]),
        )

        attributes = await self.make_attributes(self.nodes[0])
        self.assertTrue(
            len(attributes) == 0,
            "There should be no attribute in the DB of the attester.",
        )
    ...
    async def test_post_allow_verify(self):
        ...
        self.assertIn(
            "success",
            response,
            "The attestion could not be unlocked: success not in JSON response",
        )
        self.assertTrue(
            response["success"],
            "The attestation could not be unlocked: not successful.",
        )
