...
from python_project.REST.rest_manager import RESTManager
...
class MockRestIPv8(object):
    def __init__(self, crypto_curve, overlay_classes, *args, **kwargs):
        ...
        self.overlays = [
            overlay_cls(self.my_peer, self.endpoint, self.network, *args, **kwargs)
...
class RESTTestBase(TestBase):
    ...
    async def initialize(self, overlay_classes, node_count, *args, **kwargs):
        ...
        self.nodes = [
            await self.create_node(*args, **kwargs) for _ in range(node_count)
        ]
    ...
    async def create_node(self, *args, **kwargs):
        ipv8 = MockRestIPv8(
            u"curve25519", overlay_classes=self.overlay_classes, *args, **kwargs
        )
    ...
    async def make_request(
        self, node, endpoint, request_type, arguments, json_response=True
    ):
        """
        ...
        async with ClientSession() as session:
            async with session.request(
                request_type, url, params=arguments, headers=headers
            ) as response:
                return (
