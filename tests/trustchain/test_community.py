...
from tests.trustchain.test_block import TestBlock
from bami.trustchain.block import TrustChainBlock, UNKNOWN_SEQ
from bami.trustchain.caches import CrawlRequestCache
from bami.trustchain.community import TrustChainCommunity
from bami.trustchain.listener import BlockListener
...
class TestTrustChainCommunity(TestBase):
    ...
    async def test_sign_half_block(self):
        ...
        self.overlay(0).sign_block(
            self.peer(1), public_key=self.key_bin(1), block_type=b"test", transaction={}
        )
        ...
        for node_nr in [0, 1]:
            ...
            self.assertEqual(
                self.persistence(node_nr).get(self.key_bin(0), 1).link_sequence_number,
                UNKNOWN_SEQ,
            )
    ...
    async def test_sign_full_block(self):
        ...
        """
        block, link_block = await self.overlay(0).sign_block(
            self.peer(1), public_key=self.key_bin(1), block_type=b"test", transaction={}
        )
        ...
        for node_nr in [0, 1]:
            ...
            self.assertEqual(
                self.persistence(node_nr).get(self.key_bin(1), 1).link_sequence_number,
                1,
            )
    ...
    async def test_counter_tx(self):
        ...
        for node_nr in [0, 1]:
            self.assertDictEqual(
                self.nodes[node_nr]
                .overlay.persistence.get(self.key_bin(1), 1)
                .transaction,
                {"a": "b"},
            )
    ...
    async def test_crawl(self):
        ...
        self.assertEqual(
            self.persistence(1).get(self.key_bin(0), 1).link_sequence_number,
            UNKNOWN_SEQ,
        )
    ...
    async def test_crawl_no_blocks(self):
        ...
        response = await self.overlay(1).send_crawl_request(
            self.my_peer(0), self.key_bin(0), 1, 1
        )
    ...
    async def test_crawl_lowest_unknown(self):
        ...
        """
        for _ in [0, 1, 2]:
            await self.overlay(0).sign_block(
                self.peer(1),
                public_key=self.key_bin(1),
                block_type=b"test",
                transaction={},
            )

        self.persistence(1).execute(
    ...
    async def test_crawl_pair(self):
        ...
        self.assertEqual(
            self.persistence(2).get(self.key_bin(0), 1).link_sequence_number,
            UNKNOWN_SEQ,
        )
        self.assertEqual(
            self.persistence(2).get(self.key_bin(1), 1).link_sequence_number, 1
        )
    ...
    async def test_parallel_blocks(self):
        ...
        for node_nr in [0, 1]:
            ...
            self.assertEqual(
                self.persistence(node_nr).get(self.key_bin(1), 2).link_sequence_number,
                second,
            )
    ...
    async def test_retrieve_missing_block(self):
        ...
        self.endpoint(0).close()
        signed1 = self.overlay(0).sign_block(
            self.peer(1), public_key=self.key_bin(1), block_type=b"test", transaction={}
        )
        ...
        self.endpoint(0).open()
        signed2 = self.overlay(0).sign_block(
            self.peer(1), public_key=self.key_bin(1), block_type=b"test", transaction={}
        )
    ...
    async def test_intro_response_crawl(self):
        ...
        self.assertEqual(
            self.persistence(1).get_latest(self.key_bin(0)).sequence_number, 1
        )
    ...
    async def test_half_block_link_block(self):
        ...
        # Create a Link Block
        link_block, _ = await self.overlay(1).create_link(
            block, b"link", additional_info={"a": 1, "b": 2}
        )
    ...
    async def test_double_spend(self):
        ...
        block1, block2 = await self.overlay(0).sign_block(
            self.peer(1), public_key=self.key_bin(1), block_type=b"test", transaction={}
        )
        await self.deliver_messages()
    ...
    async def test_chain_crawl_with_gaps(self):
        ...
        created_blocks = []
        for _ in range(0, 5):
            blocks = await self.overlay(0).sign_block(
                self.peer(1),
                public_key=self.key_bin(1),
                block_type=b"test",
                transaction={},
    ...
    async def test_chain_crawl(self):
        ...
        for _ in range(4):
            self.overlay(0).sign_block(
                self.peer(1), public_key=key, block_type=b"test", transaction={}
            )
        self.endpoint(0).open()
        ...
        test_blocks = self.persistence(1).get_latest_blocks(
            self.key_bin(0), block_types=[b"test"]
        )
    ...
    async def test_crawl_linked_block(self):
        ...
        """
        await self.overlay(1).sign_block(
            self.peer(0), public_key=self.key_bin(0), block_type=b"test", transaction={}
        )
