    ...
    def test_connected_users(self):
        ...
        for i in range(0, 5):
            block = TrustChainBlock.create(
                b"test", {b"id": i}, self.db, self.public_key, link=random_blocks[i]
            )
            self.db.add_block(block)
...
class TestTrustChainDB(TestBase):
    ...
    def test_upgrade_blob_text(self):
        ...
        self.db.execute(
            "INSERT INTO blocks VALUES('test', '{}', '1', 3, 'test', 3, '', '', 12345, 0, '');"
        )
        self.db.execute(
            "INSERT INTO blocks VALUES('test', '{}', X'31', 3, 'test', 3, '', '', 12345, 0, '');"
        )
