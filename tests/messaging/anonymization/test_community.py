...
from tests.mocking.endpoint import MockEndpointListener
from tests.mocking.exit_socket import MockTunnelExitSocket
from tests.mocking.ipv8 import MockIPv8
from python_project.messaging.anonymization.community import (
    TunnelCommunity,
    TunnelSettings,
)
from python_project.messaging.anonymization.endpoint import TunnelEndpoint
from python_project.messaging.anonymization.tunnel import (
...
from python_project.util import cast_to_bin, succeed
...
class TestTunnelCommunity(TestBase):
    ...
    async def test_introduction_as_exit(self):
        ...
        self.assertIn(
            ...
            self.nodes[1].overlay.get_candidates(PEER_FLAG_EXIT_ANY),
        )
        self.assertNotIn(
            self.nodes[1].my_peer,
            self.nodes[0].overlay.get_candidates(PEER_FLAG_EXIT_ANY),
        )
    ...
    async def test_introduction_as_exit_noway(self):
        ...
        self.assertEqual(
            len(self.nodes[0].overlay.get_candidates(PEER_FLAG_EXIT_ANY)), 0
        )
        self.assertEqual(
            len(self.nodes[1].overlay.get_candidates(PEER_FLAG_EXIT_ANY)), 0
        )
    ...
    async def test_create_circuit_multiple_calls(self):
        ...
        self.assertEqual(
            len(self.nodes[0].overlay.find_circuits(state=CIRCUIT_STATE_EXTENDING)), 1
        )
    ...
    async def test_destroy_circuit_from_originator(self):
        ...
        await self.nodes[0].overlay.remove_circuit(
            list(self.nodes[0].overlay.circuits.keys())[0], destroy=True
        )
        await self.deliver_messages()
    ...
    async def test_destroy_circuit_from_exit(self):
        ...
        await self.nodes[2].overlay.remove_exit_socket(
            list(self.nodes[2].overlay.exit_sockets.keys())[0], destroy=True
        )
        await self.deliver_messages()
    ...
    async def test_destroy_circuit_from_relay(self):
        ...
        self.nodes[1].overlay.remove_relay(
            list(self.nodes[1].overlay.relay_from_to.keys())[0], destroy=True
        )
        await self.deliver_messages()
    ...
    async def test_destroy_circuit_bad_id(self):
        ...
        await self.nodes[0].overlay.remove_circuit(
            list(self.nodes[0].overlay.circuits.keys())[0] + 1, destroy=True
        )
        await self.deliver_messages()
    ...
    async def test_tunnel_data(self):
        ...
        future = Future()
        ep_listener.on_packet = lambda packet: ep_listener.received_packets.append(
            packet
        ) or future.set_result(None)
        await future
    ...
    async def test_create_two_circuit(self):
        ...
        self.assertEqual(
            ...
            + len(self.nodes[2].overlay.exit_sockets),
            2,
        )
    ...
    async def test_reuse_partial_circuit(self):
        ...
        circuit = list(self.nodes[0].overlay.circuits.values())[0]
        self.assertEqual(
            [h.mid for h in circuit.hops], [self.nodes[1].overlay.my_peer.mid]
        )
        ...
        cache.candidates[
            self.nodes[3].overlay.my_peer.public_key.key_to_bin()
        ] = self.nodes[3].overlay.my_peer

        # Retry to extend the circuit
        circuit.required_exit = None
        self.nodes[0].overlay.send_extend(
            circuit, [self.nodes[3].overlay.my_peer.public_key.key_to_bin()], 1
        )
        await self.deliver_messages()

        # Circuit should now be 0 -> 1 -> 3
        self.assertEqual(
            [h.mid for h in circuit.hops],
            [self.nodes[1].overlay.my_peer.mid, self.nodes[3].overlay.my_peer.mid],
        )
    ...
    async def test_reuse_partial_circuit_first_hop(self):
        ...
        # Retry to extend the circuit
        self.nodes[0].overlay.send_initial_create(
            circuit, [self.nodes[3].overlay.my_peer], 1
        )
        await self.deliver_messages()
        ...
        self.assertEqual(
            [h.mid for h in circuit.hops],
            [self.nodes[3].overlay.my_peer.mid, self.nodes[2].overlay.my_peer.mid],
        )
        self.assertEqual(circuit.unverified_hop, None)
        self.assertEqual(self.nodes[0].overlay.tunnels_ready(2), 1.0)
    ...
    async def test_tunnel_endpoint_anon(self):
        ...
        self.nodes[2].overlay.exit_sockets[
            exit_socket.circuit_id
        ] = MockTunnelExitSocket(exit_socket)
        ...
        self.nodes[0].overlay.endpoint = endpoint = TunnelEndpoint(
            self.nodes[0].overlay.endpoint
        )
