...
from python_project.messaging.serialization import PackError, Serializable, Serializer
...
class TestSerializable(Serializable):
    ...
    @classmethod
    def from_unpack_list(cls, *args) -> TestSerializable:
...
class TestSerializer(TestCase):
    ...
    def test_pack_multiple_byte_256(self) -> None:
        """
    ...
    def test_unpack_multiple_short_from_byte(self) -> None:
        """
    ...
    def test_unpack_serializables_list_short_from_byte(self) -> None:
        """
        ...
        self.assertRaises(
            PackError,
            self.serializer.unpack_to_serializables,
            [TestSerializable],
            serialized,
        )
    ...
    def test_get_formats(self) -> None:
        ...
        for format in formats:
            ...
            self.assertTrue(
                hasattr(packer, "pack"), msg="%s has no pack() method" % pack_name
            )
            self.assertTrue(
                callable(getattr(packer, "pack")),
                msg="%s.pack is not a method" % pack_name,
            )
            self.assertTrue(
                hasattr(packer, "unpack_from"),
                msg="%s has no unpack_from() method" % pack_name,
            )
            self.assertTrue(
                callable(getattr(packer, "unpack_from")),
                msg="%s.unpack_from is not a method" % pack_name,
            )
    ...
    def test_add_format(self) -> None:
        ...
        [unserialized], _ = self.serializer.unpack(
            "my_cool_format", serialized
        )  # little-endian, unpacked as 00 01 = 1
        unpack_other_end, _ = self.serializer.unpack(
            "H", serialized
        )  # big-endian, unpacked as 01 00 = 256
    ...
    def test_ez_pack_serializables(self) -> None:
        """
        ...
        deserialized1, deserialized2 = self.serializer.ez_unpack_serializables(
            [TestSerializable, TestSerializable], data
        )
    ...
    def test_ez_unpack_serializables_extra_data(self) -> None:
        """
        ...
        self.assertRaises(
            PackError,
            self.serializer.ez_unpack_serializables,
