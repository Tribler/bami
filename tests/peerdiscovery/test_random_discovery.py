...
class TestRandomWalk(TestBase):
    def setUp(self):
        ...
        self.strategies = [
            RandomWalk(self.overlays[i], reset_chance=0) for i in range(node_count)
        ]
    ...
    async def test_take_step_into(self):
        ...
        """
        self.overlays[0].network.add_verified_peer(self.overlays[1].my_peer)
        self.overlays[0].network.discover_address(
            self.overlays[1].my_peer,
            self.overlays[2].endpoint.wan_address,
