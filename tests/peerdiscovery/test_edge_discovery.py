...
from python_project.peerdiscovery.discovery import EdgeWalk
...
class TestEdgeWalk(TestBase):
    def setUp(self):
        ...
        self.strategies = [
            EdgeWalk(self.overlays[i], neighborhood_size=1) for i in range(node_count)
        ]
    ...
    async def test_take_step(self):
        ...
        self.overlays[0].network.discover_services(
            self.overlays[1].my_peer, [self.overlays[1].master_peer.mid,]
        )
        self.overlays[1].network.add_verified_peer(self.overlays[2].my_peer)
        self.overlays[1].network.discover_services(
            self.overlays[2].my_peer, [self.overlays[2].master_peer.mid,]
        )
        ...
        self.strategies[
            0
        ].take_step()  # Find out the neighbor has been introduced and walk to it
        await self.deliver_messages()
    ...
    async def test_complete_edge(self):
        ...
        self.strategies[
            0
        ].edge_timeout = 0.15  # Finish the edge, but allow the network to walk
        self.overlays[0].network.add_verified_peer(self.overlays[1].my_peer)
        self.overlays[0].network.discover_services(
            self.overlays[1].my_peer, [self.overlays[1].master_peer.mid,]
        )
