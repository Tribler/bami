...
from tests.mocking.community import MockCommunity
from python_project.community import _DEFAULT_ADDRESSES
...
from python_project.peerdiscovery.payload import DiscoveryIntroductionRequestPayload
...
class TestDiscoveryCommunity(TestBase):
    ...
    async def test_deprecated_introduction(self):
        ...
        payload = DiscoveryIntroductionRequestPayload(
            b"a" * 20,
        ...
        auth = BinMemberAuthenticationPayload(
            self.overlays[0].my_peer.public_key.key_to_bin()
        ).to_pack_list()
        dist = GlobalTimeDistributionPayload(global_time).to_pack_list()

        packet = self.overlays[0]._ez_pack(
            self.overlays[0]._prefix, 246, [auth, dist, payload]
        )
        self.overlays[1].on_introduction_request(
            self.overlays[0].endpoint.wan_address, packet
        )
    ...
    async def test_cross_peer(self):
        ...
        self.overlays[0].network.register_service_provider(
            custom_master_peer.mid, custom_overlay
        )
        ...
        discovered = reduce(
            lambda a, b: a | b,
            self.overlays[1].network.services_per_peer.values(),
            set(),
        )

        self.assertEqual(len(self.overlays[1].network.services_per_peer), 2)
        self.assertSetEqual(
            discovered, {MockCommunity.master_peer.mid, custom_master_peer.mid}
        )
