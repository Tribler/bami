...
class TestNetwork(asynctest.TestCase):
    ...
    def test_discover_address(self):
        ...
        self.assertIn(
            self.peers[1].address, self.network.get_introductions_from(self.peers[0])
        )
    ...
    def test_discover_address_known_parent_deceased(self):
        ...
        self.assertNotIn(self.peers[1], self.network.verified_peers)
        self.assertIn(
            self.peers[1].address, self.network.get_introductions_from(self.peers[2])
        )
    ...
    def test_discover_address_multiple(self):
        ...
        for other in [1, 2]:
            self.assertIn(
                self.peers[other].address, self.network.get_walkable_addresses()
            )
    ...
    def test_get_introductions_from_cache_no_refresh(self):
        ...
        self.assertListEqual(
            [self.peers[0], self.peers[1]], list(self.network.reverse_intro_lookup)
        )
    ...
    def test_get_verified_by_address(self):
        ...
        self.assertEqual(
            self.peers[0], self.network.get_verified_by_address(self.peers[0].address)
        )
    ...
    def test_get_verified_by_address_cache_pop(self):
        ...
        self.network.get_verified_by_address(
            self.peers[1].address
        )  # Cache stack: [1] (full)
        self.assertListEqual(
            [self.peers[1].address], list(self.network.reverse_ip_lookup)
        )

        self.network.get_verified_by_address(
            self.peers[0].address
        )  # Cache stack: [0] (full)
        self.assertListEqual(
            [self.peers[0].address], list(self.network.reverse_ip_lookup)
        )
    ...
    def test_get_verified_by_address_cache_refresh(self):
        ...
        self.network.get_verified_by_address(
            self.peers[0].address
        )  # Cache stack: [1, 0] (full)
        self.network.get_verified_by_address(
            self.peers[1].address
        )  # Cache stack: [0, 1] (full)

        self.assertListEqual(
            [self.peers[0].address, self.peers[1].address],
            list(self.network.reverse_ip_lookup),
        )
    ...
    def test_get_walkable_by_service(self):
        ...
        self.assertEqual(
            [self.peers[1].address], self.network.get_walkable_addresses(service)
        )
