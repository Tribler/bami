...
from python_project.attestation.trustchain.block import TrustChainBlock
from python_project.attestation.trustchain.caches import CrawlRequestCache
from python_project.attestation.trustchain.community import (
    TrustChainCommunity,
    UNKNOWN_SEQ,
)
...
class TestTrustChainCommunity(TestBase):
    ...
    def create_node(self):
        return MockIPv8(
            u"curve25519", TrustChainCommunity, working_directory=u":memory:"
        )
    ...
    async def test_sign_half_block(self):
        ...
        his_pubkey = list(self.nodes[0].network.verified_peers)[
            0
        ...
        for node_nr in [0, 1]:
            self.assertIsNotNone(
                self.nodes[node_nr].overlay.persistence.get(my_pubkey, 1)
            )
            self.assertEqual(
                self.nodes[node_nr]
                .overlay.persistence.get(my_pubkey, 1)
    ...
    async def test_sign_full_block(self):
        ...
        block, link_block = await self.nodes[0].overlay.sign_block(
            list(self.nodes[0].network.verified_peers)[0],
            public_key=his_pubkey,
            block_type=b"test",
            transaction={},
        ...
        for node_nr in [0, 1]:
            self.assertIsNotNone(
                self.nodes[node_nr].overlay.persistence.get(his_pubkey, 1)
            )
            self.assertEqual(
                self.nodes[node_nr]
                .overlay.persistence.get(his_pubkey, 1)
                .link_sequence_number,
                1,
            )
    ...
    async def test_get_linked(self):
        ...
        for node_nr in [0, 1]:
            ...
            self.assertEqual(
                self.nodes[node_nr].overlay.persistence.get_linked(my_block), his_block
            )
            self.assertEqual(
                self.nodes[node_nr].overlay.persistence.get_linked(his_block), my_block
            )
    ...
    async def test_crawl(self):
        ...
        self.assertEqual(
            self.nodes[1].overlay.persistence.get(my_pubkey, 1).link_sequence_number,
            UNKNOWN_SEQ,
        )
    ...
    async def test_crawl_no_blocks(self):
        ...
        response = await self.nodes[1].overlay.send_crawl_request(
            self.nodes[0].my_peer, my_pubkey, 1, 1
        )
    ...
    async def test_crawl_negative_index(self):
        ...
        self.nodes[1].overlay.send_crawl_request(
            self.nodes[0].my_peer, my_pubkey, -1, -1
        )
    ...
    async def test_crawl_lowest_unknown(self):
        ...
        for _ in [0, 1, 2]:
            await self.nodes[0].overlay.sign_block(
                list(self.nodes[0].network.verified_peers)[0],
                public_key=his_pubkey,
                block_type=b"test",
                transaction={},
        ...
        self.nodes[1].overlay.persistence.execute(
            u"DELETE FROM blocks WHERE sequence_number = 2 AND public_key = ?",
    ...
    async def test_crawl_pair(self):
        ...
        my_pubkey = self.nodes[0].my_peer.public_key.key_to_bin()
        await self.nodes[2].overlay.send_crawl_request(
            self.nodes[0].my_peer, my_pubkey, 1, 1
        )

        # Check whether we have both blocks now
        self.assertEqual(
            self.nodes[2].overlay.persistence.get(my_pubkey, 1).link_sequence_number,
            UNKNOWN_SEQ,
        )
        self.assertEqual(
            self.nodes[2].overlay.persistence.get(his_pubkey, 1).link_sequence_number, 1
        )
    ...
    async def test_parallel_blocks(self):
        ...
        #  0@block#1 <-> 1@block#2
        first = (
            self.nodes[1].overlay.persistence.get(his_pubkey, 1).link_sequence_number
        )
        ...
        for node_nr in [0, 1]:
            ...
            self.assertIsNotNone(
                self.nodes[node_nr].overlay.persistence.get(his_pubkey, 2)
            )
            self.assertEqual(
                self.nodes[node_nr]
                .overlay.persistence.get(his_pubkey, 2)
                .link_sequence_number,
                second,
            )
    ...
    async def test_retrieve_missing_block(self):
        ...
        self.nodes[0].endpoint.close()
        signed1 = self.nodes[0].overlay.sign_block(
            list(self.nodes[0].network.verified_peers)[0],
            public_key=his_pubkey,
            block_type=b"test",
            transaction={},
        ...
        self.nodes[0].endpoint.open()
        signed2 = self.nodes[0].overlay.sign_block(
            list(self.nodes[0].network.verified_peers)[0],
            public_key=his_pubkey,
            block_type=b"test",
            transaction={},
    ...
    async def test_send_block_pair(self):
        ...
        block1 = TestBlock()
        block2 = TestBlock()
        self.nodes[0].overlay.send_block_pair(
            block1, block2, list(self.nodes[0].network.verified_peers)[0].address
        )
    ...
    async def test_intro_response_crawl(self):
        ...
        self.assertEqual(
            self.nodes[1].overlay.persistence.get_latest(my_pubkey).sequence_number, 1
        )
    ...
    async def test_empty_crawl(self):
        ...
        """
        my_pubkey = self.nodes[0].my_peer.public_key.key_to_bin()
        await self.nodes[1].overlay.send_crawl_request(
            self.nodes[0].my_peer, my_pubkey, 1, 1
        )
    ...
    async def test_half_block_link_block(self):
        ...
        self.assertIsNotNone(
            self.nodes[0].overlay.persistence.get(source_peer_pubkey, 1)
        )
        block = self.nodes[1].overlay.persistence.get(source_peer_pubkey, 1)
        self.assertIsNotNone(block)

        # Create a Link Block
        link_block, _ = await self.nodes[1].overlay.create_link(
            block, b"link", additional_info={b"a": 1, b"b": 2}
        )
    ...
    async def test_link_block_multiple(self):
        ...
        self.nodes[1].overlay.create_link(
            block, b"link", additional_info={b"a": 1, b"b": 2}
        )
        self.nodes[1].overlay.create_link(
            block, b"link", additional_info={b"a": 2, b"b": 3}
        )
        await self.deliver_messages()

        self.assertEqual(
            len(self.nodes[0].overlay.persistence.get_all_linked(source_block)), 2
        )
    ...
    def test_db_remove(self):
        ...
        self.assertEqual(
            self.nodes[0].overlay.persistence.get_number_of_known_blocks(), 5
        )
    ...
    def test_database_cleanup(self):
        ...
        self.nodes[0].overlay.do_db_cleanup()
        self.assertEqual(
            self.nodes[0].overlay.persistence.get_number_of_known_blocks(), 3
        )
    ...
    async def test_double_spend(self):
        ...
        block1, block2 = await self.nodes[0].overlay.sign_block(
            list(self.nodes[0].network.verified_peers)[0],
            public_key=his_pubkey,
            block_type=b"test",
            transaction={},
    ...
    async def test_chain_crawl_with_gaps(self):
        ...
        created_blocks = []
        for _ in range(0, 5):
            blocks = await self.nodes[0].overlay.sign_block(
                list(self.nodes[0].network.verified_peers)[0],
                public_key=his_pubkey,
                block_type=b"test",
                transaction={},
        ...
        self.assertEqual(
            self.nodes[1].overlay.persistence.get_number_of_known_blocks(), 10
        )
    ...
    async def test_chain_crawl(self):
        ...
        test_blocks = self.nodes[1].overlay.persistence.get_latest_blocks(
            node0_pubkey, block_types=[b"test"]
        )
    ...
    async def test_chain_crawl_unknown_length(self):
        ...
        self.assertEqual(
            self.nodes[1].overlay.persistence.get_number_of_known_blocks(), 4
        )
        ...
        self.assertEqual(
            self.nodes[1].overlay.persistence.get_number_of_known_blocks(), 7
        )
    ...
    async def test_crawl_linked_block(self):
        ...
        """
        his_pubkey = list(self.nodes[1].network.verified_peers)[
            0
        ].public_key.key_to_bin()
        await self.nodes[1].overlay.sign_block(
            list(self.nodes[1].network.verified_peers)[0],
            public_key=his_pubkey,
            block_type=b"test",
            transaction={},
        )
        ...
        self.add_node_to_experiment(self.create_node())
        await self.nodes[2].overlay.send_crawl_request(
            self.nodes[0].my_peer, self.nodes[0].my_peer.public_key.key_to_bin(), 1, 1
        )

        # Peer 2 should have 2 blocks now
        self.assertEqual(
            self.nodes[2].overlay.persistence.get_number_of_known_blocks(), 2
        )
    ...
    async def test_process_block_unrelated_block(self):
        ...
        block1 = TestBlock()
        try:
            result = await self.nodes[1].overlay.process_half_block(
                block1, self.nodes[0].my_peer
            )
        except RuntimeError:
            pass
    ...
    async def test_process_block(self):
        ...
        block = self.nodes[0].overlay.persistence.get_latest(
            self.nodes[0].my_peer.public_key.key_to_bin()
        )
        self.nodes[0].endpoint.open()

        blocks = await self.nodes[1].overlay.process_half_block(
            block, self.nodes[0].my_peer
        )
