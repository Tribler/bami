...
class TestTrustChainBlock(asynctest.TestCase):
    ...
    def test_hash(self):
        ...
        self.assertEqual(
            block.hash,
            b"9X\xdeb\x92g\x10W\t\xdf\xf6\x98\xc46\xdaU\x19+6\x1b\xf4\xaei"
    ...
    def test_create_genesis(self):
        ...
        db = MockDatabase()
        block = TrustChainBlock.create(
            b"test", {b"id": 42}, db, key.pub().key_to_bin(), link=None
        )
    ...
    def test_create_next(self):
        ...
        db.add_block(prev)
        block = TrustChainBlock.create(
            b"test", {b"id": 42}, db, prev.public_key, link=None
        )
    ...
    def test_create_link_next(self):
        ...
        db.add_block(link)
        block = TrustChainBlock.create(
            b"test", {b"id": 42}, db, prev.public_key, link=link
        )
        self.assertEqual(block.previous_hash, prev.hash)
        self.assertEqual(block.sequence_number, 2)
        self.assertEqual(block.public_key, prev.public_key)
