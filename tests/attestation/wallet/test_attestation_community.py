...
from python_project.attestation.wallet.database import AttestationsDB
from python_project.attestation.wallet.pengbaorange.structs import PengBaoAttestation
from python_project.attestation.wallet.primitives.structs import BonehPrivateKey


class TestCommunity(TestBase):

    private_key = BonehPrivateKey.unserialize(
        unhexlify(
    ...
    def create_node(self):
        return MockIPv8(
            u"curve25519", AttestationCommunity, working_directory=u":memory:"
        )
    ...
    async def test_request_attestation_callback(self):
        ...
        self.nodes[1].overlay.request_attestation(
            self.nodes[0].overlay.my_peer, "MyAttribute", TestCommunity.private_key
        )
    ...
    async def test_request_attestation(self):
        ...
        self.nodes[0].overlay.set_attestation_request_callback(
            lambda x, y, z: b"AttributeValue"
        )
    ...
    async def test_verify_attestation(self):
        ...
        self.nodes[0].overlay.database.insert_attestation(
            attestation, attestation_hash, TestCommunity.private_key, "id_metadata"
        )
        self.nodes[0].overlay.attestation_keys[attestation_hash] = (
            TestCommunity.private_key,
            "id_metadata",
        )
        ...
        self.nodes[1].overlay.verify_attestation_values(
    ...
    async def test_reqandverif_attestation(self):
        ...
        callback.called = False
        self.nodes[0].overlay.verify_attestation_values(
    ...
    async def test_verify_attestation_big(self):
        ...
        self.nodes[0].overlay.database.insert_attestation(
            attestation, attestation_hash, TestCommunity.private_key, "id_metadata_big"
        )
        self.nodes[0].overlay.attestation_keys[attestation_hash] = (
            TestCommunity.private_key,
            "id_metadata_big",
    ...
    async def test_verify_attestation_range(self):
        ...
        with open(filename, "r") as f:
            serialized = unhexlify(f.read().strip())
        attestation = PengBaoAttestation.unserialize_private(
            self.private_key, serialized, "id_metadata_range_18plus"
        )
    ...
    def test_load_key(self):
        ...
        attestation = BonehAttestation(
            TestCommunity.private_key.public_key(), [], "id_metadata"
        )
        overlay.on_attestation_complete(
            attestation,
            TestCommunity.private_key,
            None,
            "test",
        ...
        self.nodes[0].overlay.__init__(
            self.nodes[0].my_peer,
            self.nodes[0].endpoint,
            self.nodes[0].network,
