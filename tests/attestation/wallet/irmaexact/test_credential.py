...
from python_project.attestation.wallet.irmaexact.gabi.builder import (
...
from python_project.attestation.wallet.irmaexact.gabi.credential import Credential
...
class TestCredential(asynctest.TestCase):
    ...
    def test_show_proof(self):
        ...
        """
        signature = SignMessageBlock(
            self.testPrivK, self.testPubK, self.testAttributes1
        )
    ...
    def test_construct_credential_challenge(self):
        ...
        self.assertTrue(
            Verify(commit_msg.Proofs, [self.testPubK1], context, nonce1, False)
        )

        issuer = Issuer(self.testPrivK1, self.testPubK1, context)
        msg = issuer.IssueSignature(
            commit_msg.Proofs[0].U, self.testAttributes1, nonce2
        )
    ...
    def test_construct_credential_challenge_pcommit(self):
        ...
        cr = {
            ...
            u"attributes": {u"test1": u"value1", u"test2": u"value2"},
        }

        s = 1234
        s_randomizer = 5678
        pcommit = ProofPCommitment(
            FP2Value(issuer.Pk.N, issuer.Pk.R[0]).intpow(s).a,
        ...
        secondaryChallenge = createChallenge(
            challenge_verif, challenge_verif, [A, Z], False
        )
    ...
    def test_show_proof_list(self):
        ...
        builders = [
            cred1.CreateDisclosureProofBuilder([1, 2]),
            cred2.CreateDisclosureProofBuilder([1, 3]),
        ]
        prooflist = BuildProofList(builders, context, nonce1, False)

        self.assertTrue(
            Verify(prooflist, [issuer1.Pk, issuer2.Pk], context, nonce1, False)
        )
    ...
    def test_issue_show_random(self):
        ...
        self.assertTrue(
            Verify(commitMsg.Proofs, [issuer1.Pk, issuer2.Pk], context, nonce1, False)
        )
