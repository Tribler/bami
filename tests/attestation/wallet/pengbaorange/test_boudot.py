...
from python_project.attestation.wallet.pengbaorange.boudot import EL, SQR
...
class TestBoudot(asynctest.TestCase):
    ...
    pk = BonehPrivateKey.unserialize(
        unhexlify(
            "0109649b2a7d7992b008d1010958f4d560346bb392330109117572cef25"
            "23be2fe0109264c97f1868fd4b18b010908792e2bf8bcaa077f0108baa1"
            "b2ff06a0cfd50104c772a78f"
        )
    ).public_key()
    bitspace = 32
    a, b = 0, 18
    ...
    el = EL.unserialize(
        unhexlify(
            ""
        )
    )[0]
    ...
    sqr = SQR.unserialize(
        unhexlify(
            "0109649b2a7d7992b008d101092ef4b2b3890136f39d0109300f2cb5896dcadc4901012a0a16027d"
    ...
    def test_el_serialize(self):
        ...
        """
        el = EL(
            68174117739048401651990398043836840253231911962332258219191049320869958258614,
    ...
    def test_sqr(self):
        ...
        self.assertTrue(
            sqr.check(self.pk.g, self.pk.h, self.pk.g.intpow(16) * self.pk.h.intpow(81))
        )
    ...
    def test_sqr_modify_shadow_commitment(self):
        ...
        sqr = SQR.create(4, 81, self.pk.g, self.pk.h, self.b, self.bitspace)
        self.assertFalse(
            sqr.check(self.pk.g, self.pk.h, self.pk.g.intpow(9) * self.pk.h.intpow(81))
        )
    ...
    def test_sqr_serialize(self):
        ...
        """
        el = EL(
            77692238748522549651683873494184958254240308467748145113388211342290056191907,
        ...
        sqr = SQR(
            FP2Value(self.pk.g.mod, 866182580282760557469, 886537163949459823689), el
        )
