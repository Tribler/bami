...
from python_project.dht.routing import Node, RoutingTable
...
class TestDHTDiscoveryCommunity(TestBase):
    ...
    async def test_connect_peer(self):
        ...
        self.nodes[1].overlay.store[self.nodes[0].my_peer.mid].append(
            self.nodes[0].my_peer
        )
        self.nodes[0].overlay.store_for_me[self.nodes[0].my_peer.mid].append(
    ...
    def test_ping_all(self):
        self.nodes[0].overlay.ping = lambda n: setattr(self, "pinged", n) or succeed(
            None
        )
