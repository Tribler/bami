...
from python_project.dht.provider import DHTCommunityProvider
from python_project.dht.routing import NODE_LIMIT_QUERIES, Node, RoutingTable, distance
from python_project.messaging.anonymization.tunnel import IntroductionPoint
...
class TestDHTCommunity(TestBase):
    def setUp(self):
        ...
        self.key = b"\x00" * 20
        self.value = b"test"
        self.value_in_store = self.nodes[0].overlay.serialize_value(
            self.value, sign=False
        )
        self.signed_in_store = self.nodes[0].overlay.serialize_value(
        ...
        for node1 in self.nodes:
            for node2 in self.nodes:
                ...
                node1.overlay.tokens[dht_node2] = (
                    now,
                    node2.overlay.generate_token(dht_node1),
                )
    ...
    async def test_ping_pong(self):
        await self.introduce_nodes()
        node = await self.nodes[0].overlay.ping(
            Node(self.nodes[1].my_peer.key, self.nodes[1].my_peer.address)
        )
    ...
    async def test_store_value(self):
        ...
        self.assertEqual(
            self.nodes[1].overlay.storage.get(self.key), [self.value_in_store]
        )

    async def test_store_value_fail(self):
        await self.introduce_nodes()
        self.nodes[0].overlay.routing_table = RoutingTable(
            self.nodes[0].overlay.my_node_id
        )
    ...
    async def test_find_nodes(self):
        ...
        self.assertSetEqual(
            ...
            set(
                [
                    Node(n.my_peer.key.pub().key_to_bin(), n.my_peer.address)
    ...
    async def test_find_values_signed(self):
        ...
        values = await self.nodes[0].overlay.find_values(self.key)
        self.assertIn(
            (self.value, self.nodes[0].my_peer.public_key.key_to_bin()), values
        )
    ...
    async def test_caching(self):
        ...
        self.nodes.sort(
            key=lambda n: distance(n.overlay.my_node_id, self.key), reverse=True
        )

        self.nodes[0].overlay.on_node_discovered(
            self.nodes[1].my_peer.key, self.nodes[1].my_peer.address
        )
        self.nodes[1].overlay.on_node_discovered(
            self.nodes[2].my_peer.key, self.nodes[2].my_peer.address
        )

        self.nodes[2].overlay.storage.put(self.key, self.value_in_store)
        await self.nodes[0].overlay.find_values(self.key)
        await self.deliver_messages(0.2)
        self.assertEqual(
            self.nodes[1].overlay.storage.get(self.key), [self.value_in_store]
        )
    ...
    async def test_refresh(self):
        ...
        bucket = self.nodes[0].overlay.routing_table.get_bucket(
            self.nodes[1].overlay.my_node_id
        )
        bucket.last_changed = 0

        self.nodes[0].overlay.find_values = lambda *args: setattr(
            self, "is_called", True
        ) or succeed([])
    ...
    async def test_token(self):
        ...
        await self.introduce_nodes()
        await self.nodes[0].overlay.store_on_nodes(
            self.key, [self.value_in_store], [dht_node]
        )
        await self.deliver_messages()
        self.assertEqual(
            self.nodes[1].overlay.storage.get(self.key), [self.value_in_store]
    ...
    async def test_provider(self):
        ...
        await dht_provider_1.announce(
            b"a" * 20, IntroductionPoint(self.nodes[0].overlay.my_peer, "\x01" * 20)
        )
        await dht_provider_2.announce(
            b"a" * 20, IntroductionPoint(self.nodes[1].overlay.my_peer, "\x02" * 20)
        )
    ...
    async def test_rate_limit(self):
        ...
        num_queries = len(
            self.nodes[1].overlay.routing_table.get(node0.id).last_queries
        )
...
class TestDHTCommunityXL(TestBase):
    ...
    def get_closest_nodes(self, node_id, max_nodes=8):
        return sorted(
            self.nodes, key=lambda n: distance(n.overlay.my_node_id, node_id)
        )[:max_nodes]
